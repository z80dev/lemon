...
import { truncateLines, truncateText } from "./base.js";
import type { FormattedOutput, ToolFormatter } from "./types.js";
...
export const processFormatter: ToolFormatter = {
	...
	formatArgs(args: Record<string, unknown>): FormattedOutput {
		...
		const summary = processArgs.process_id ? `${action}: ${processArgs.process_id}` : action;
	...
	formatResult(result: unknown, args?: Record<string, unknown>): FormattedOutput {
		...
		const processId = getProcessId(result, args);
		const status =
			result && typeof result === "object" && typeof (result as ProcessResult).status === "string"

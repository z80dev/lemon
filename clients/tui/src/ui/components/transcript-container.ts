/**
 * Copied near-verbatim from oh-my-pi
 * (`packages/coding-agent/src/modes/components/transcript-container.ts`) against
 * the same pi-tui version (17.3.4). The only local edits are the import paths
 * and the removal of omp's tool-activity visibility plumbing (`addChild`
 * override + `setToolActivityVisible`), which has no counterpart here yet.
 *
 * Do not re-derive this file. The FinalizableBlock protocol, the settled-rows
 * mapping and the commit-seam bookkeeping are load-bearing for native-scrollback
 * correctness; the comments explain why each guard exists.
 */
...
export interface FinalizableBlock {
	 ...
	 * reporting finalized (e.g. `AssistantMessageComponent`: a late error line,
	 * a retro-actively attached result). The committed-scrollback render bypass
	 * only replays a block's previous rows when the version is unchanged;
	 * without this signal a post-finalize mutation would stay invisible until a
	 * global invalidation. Blocks that never mutate post-finalize simply omit
	 * the method.
	 */
	...
	/**
	 * Whether the block is a displaceable snapshot (a card a follow-up call can
	 * retract) kept unfinalized only so that retraction stays possible. Paired
...
function setBlockCommittedRows(child: Component, rows: number): void {
	(child as Component & Partial<NativeScrollbackCommittedRows>).setNativeScrollbackCommittedRows?.(
		rows,
	);
}
		...
		ViewportTailProvider
{
	...
	override setNativeScrollbackCommittedRows(rows: number): void {
		...
		for (let i = 0; i < this.children.length; i++) {
			...
			while (
				leadingTrimmedRows < segment.rawRef.length &&
				isPlainBlank(segment.rawRef[leadingTrimmedRows]!)
			) {
				leadingTrimmedRows++;
			}
			setBlockCommittedRows(
				child,
				Math.min(segment.rawRef.length, leadingTrimmedRows + committedContribution),
			);
		}
	}
	...
	override captureNativeScrollbackWidthEpoch(): unknown {
		...
		const segment =
			this.#segments.find((candidate) => !candidate.finalized) ?? this.#segments.at(-1);
	...
	override resolveNativeScrollbackWidthEpoch(boundary: unknown): number | undefined {
		...
		const currentIndex = this.#segments.findIndex(
			(segment) => segment.component === marker.segment.component,
		);
		...
		for (const captured of marker.trailingSegments) {
			...
			if (!captured.finalized || !trailing?.finalized || trailing.version !== captured.version) {
				return undefined;
			}
	...
	#mapNativeScrollbackWidthEpochRows(segment: BlockSegment, rawRows: number): number {
		...
		const contributionRows = Math.max(
			0,
			Math.min(segment.contribution.length, rawRows - leadingTrimmedRows),
		);
	...
	override getNativeScrollbackWidthEpochRows(): number | undefined {
		...
		for (const trailing of this.#segments.slice(this.#segments.indexOf(segment) + 1)) {
			rows += trailing.rowCount;
		}
		return rows;
	}
	...
	override isNativeScrollbackWidthEpochAppendOnly(boundary: unknown): boolean {
		...
		if (child.isNativeScrollbackWidthEpochAppendOnly?.(marker.childBoundary) === false)
			return false;
	 ...
	 * entered native scrollback. Callers that retract ephemeral blocks must
	 * check this: removing a block whose rows are already on the tape is an
	 * interior deletion of committed history the engine cannot express — the
	 * block must be sealed in place as history instead. A component that has
	 * never rendered has no committed rows and is safely removable.
	 */
	 ...
	 * finalized blocks poll this to stop animating — and settle on static bytes
	 * — the moment they sit above the seam, where their rows become
	 * commit-eligible native-scrollback history.
	 */
	...
	override render(width: number): readonly string[] {
		...
		// block that has not finalized must gate it: out-of-band inserts can
		// append a finalized block *below* a tool that is still awaiting its
		// result, and committing rows there would strand the tool's history rows
		// on a mid-stream preview the late result never reaches.
		...
		for (let i = 0; i < count; i++) {
			...
			const committedReusable =
				...
				previous.finalized &&
				// Post-finalize mutations bump the block version; a mismatch forces
				// a real render so the committed-prefix audit can observe and
				// re-anchor the change.
...
/**
 * Groups a run of sibling rows (a tool card's header + body, a bordered panel)
 * into a single transcript child so the container spaces it as one block — one
 * blank line above, none injected between its rows. Without this wrapper the
 * rows would be top-level children and the container would put a blank line
 * between each. It is a plain {@link Container}; the named subclass documents
 * intent and makes every manual block grouping greppable.
 */

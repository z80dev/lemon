...
import { processFormatter } from "../../src/formatters/process.js";
...
describe("processFormatter.formatArgs", () => {
	...
	it("truncates long input", () => {
		const longInput =
			"This is a very long input string that exceeds the maximum display length and should be truncated with ellipsis";
		const result = processFormatter.formatArgs({
			action: "write",
			process_id: "proc-001",
			input: longInput,
...
describe("processFormatter.formatResult - list action", () => {
	...
	it("shows table header and rows", () => {
		...
		expect(
			result.details.some(
				(d) => d.includes("proc-1") && d.includes("running") && d.includes("npm test"),
			),
		).toBe(true);
		expect(
			result.details.some(
				(d) => d.includes("proc-2") && d.includes("exited") && d.includes("npm build"),
			),
		).toBe(true);
	});

	it("handles empty process list", () => {
		const result = processFormatter.formatResult({ processes: [] }, { action: "list" });
	...
	it("handles processes without commands", () => {
		const result = processFormatter.formatResult(
			{
				processes: [{ id: "proc-1", status: "running" }],
			},
			{ action: "list" },
		);
	...
	it("truncates long commands", () => {
		const longCommand =
			"npm run very-long-script-name-that-exceeds-the-display-width --with-many-options --and-flags";
		const result = processFormatter.formatResult(
			{
				processes: [{ id: "proc-1", status: "running", command: longCommand }],
			},
			{ action: "list" },
		);
...
describe("processFormatter.formatResult - poll/log action", () => {
	...
	it("handles lines array format", () => {
		const result = processFormatter.formatResult(
			{
				lines: [{ stream: "stdout", text: "Line from lines array" }],
			},
...
describe("realistic scenarios", () => {
	it("formats process list showing multiple background jobs", () => {
		...
		expect(result.details).toContain("ID       STATUS   COMMAND");
		expect(result.details.some((d) => d.includes("web-server") && d.includes("running"))).toBe(
			true,
		);

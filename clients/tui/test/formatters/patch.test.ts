...
mock.module("node:os", () => ({
	homedir: mock(() => "/Users/testuser"),
}));

describe("patchFormatter", () => {
	describe("formatArgs", () => {
		it('shows "Applying patch (N files)" summary for single file', async () => {
			const { patchFormatter } = await import("../../src/formatters/patch.js");
...
-export default oldFunction;`;
			...
			expect(result.details.some((d) => d.startsWith("- ") && d.includes("deprecated.ts"))).toBe(
				true,
			);
...
+  timeout: 5000,
	...
	describe("formatResult - failure", () => {
		...
		it("detects error from isError field", async () => {
			...
			const result = patchFormatter.formatResult({ isError: true }, { patch_text: "" });
		...
		it("shows failure details", async () => {
			...
			const result = patchFormatter.formatResult({ error: true }, { patch_text: "" });

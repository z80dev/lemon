...
describe("shortenUrl", () => {
	...
	it("truncates long URLs to 50 characters", () => {
		const longUrl =
			"https://example.com/this/is/a/very/long/path/that/exceeds/the/maximum/length/limit";
...
describe("webFetchFormatter.formatResult", () => {
	...
	it("detects error status (>= 400)", () => {
		expect(webFetchFormatter.formatResult({ details: { status: 400 } }).isError).toBe(true);
		expect(webFetchFormatter.formatResult({ details: { status: 401 } }).isError).toBe(true);
		expect(webFetchFormatter.formatResult({ details: { status: 403 } }).isError).toBe(true);
		expect(webFetchFormatter.formatResult({ details: { status: 500 } }).isError).toBe(true);
		expect(webFetchFormatter.formatResult({ details: { status: 503 } }).isError).toBe(true);
	});

	it("does not mark success status as error", () => {
		expect(webFetchFormatter.formatResult({ details: { status: 200 } }).isError).toBe(false);
		expect(webFetchFormatter.formatResult({ details: { status: 201 } }).isError).toBe(false);
		expect(webFetchFormatter.formatResult({ details: { status: 301 } }).isError).toBe(false);
	});
...
describe("webSearchFormatter.formatArgs", () => {
	...
	it("truncates long queries in summary", () => {
		const longQuery =
			"This is a very long search query that exceeds the maximum display length for summaries";
...
describe("webSearchFormatter.formatResult", () => {
	...
	it("extracts domain correctly from URLs", () => {
		const result = webSearchFormatter.formatResult({
			details: {
				results: [{ title: "Docs", url: "https://docs.example.com/api/v2/reference?param=value" }],
			},
		});
	...
	it("truncates long query in summary", () => {
		const longQuery =
			"This is a very long search query that should be truncated in the summary display";
		const result = webSearchFormatter.formatResult(
			{
				details: {
...
describe("webFormatter (combined)", () => {
	...
	it("routes formatResult to websearch when args have query", () => {
		const result = webFormatter.formatResult({ details: { results: [] } }, { query: "test" });
...
describe("realistic scenarios", () => {
	...
	it("formats search results for documentation query", () => {
		...
		expect(
			searchResult.details.some(
				(d) => d.includes("Mocking | Guide | Vitest") && d.includes("vitest.dev"),
			),
		).toBe(true);
		expect(
			searchResult.details.some(
				(d) => d.includes("Mock Functions | Vitest") && d.includes("vitest.dev"),
			),
		).toBe(true);
		expect(
			searchResult.details.some(
				(d) => d.includes("Vitest Testing Tutorial") && d.includes("dev.to"),
			),
		).toBe(true);
	});
});

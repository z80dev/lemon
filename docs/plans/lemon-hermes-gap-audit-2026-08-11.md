...
| Discover and administer the product from one CLI | Hermes documents roughly sixty top-level families in its pinned [CLI reference](https://github.com/NousResearch/hermes-agent/blob/4f22543509d1b91dc45bcb369447126c5eb14fb7/website/docs/reference/cli-commands.md): chat, models/fallback/MoA, sessions, cron, profiles, skills, memory, approvals, backups, updates, logs, plugins, MCP, completions, and more. | Packaged [`LemonCli.CLI`](../../apps/lemon_cli/lib/lemon_cli/cli.ex) now covers setup, model/provider, gateway, doctor, config, secrets, channels, verified backup/restore, profiles/canonical chat, durable sessions, and registry-generated Bash/Zsh/Fish completion. Source `bin/lemon` additionally exposes node, send, media, policy, proofs, readiness, skill, usage, and update through Mix tasks. Supported packaged surfaces now share dispatch/help/completion metadata, while source-only operations and several runtime administration families remain outside that map. | Partial | **High** | Promote the remaining supported source-only runtime surfaces into the packaged registry with the same help/JSON/errors/completion contract. |
...
| Create durable specialist agents and collaborate among them | [Bot Mode](https://hermes-agent.nousresearch.com/docs/user-guide/bot-mode) presents profiles as bots with independent config/model/memory/skills/SOUL, canonical chats, cron routines, DMs, groups, cross-machine roster, and remote creation. Profiles also have full [CLI lifecycle](https://hermes-agent.nousresearch.com/docs/reference/profile-commands). | Lemon now has a first-class profile record over its existing router agents and named nodes: isolated homes/workspaces, create/list/show/clone/rename/credential-safe-export/recoverable-delete, stable `agent:<id>:main` chats, named-node routing, a node-aware roster, packaged CLI, and control-plane RPCs. Group rooms, profile-scoped cron, import/restore, and TUI/Web profile editors remain absent. | **Near** for lifecycle/canonical chat; partial for collaboration UI | **High** | Add the TUI/Web profile picker/editor and profile-scoped automation next; design group rooms only after those shared lifecycle APIs have real client use. |
...
| Interrupt, steer, or redirect a running agent | Hermes distinguishes hard interrupt, non-canceling steer, and model-request redirect. | Lemon implements the same semantics through router queue modes, `CodingAgent.Session`, and `LemonAgent.Loop`; named-node runs carry bounded steer/redirect controls to the actual destination session and wait for authenticated acceptance. Control-plane chat methods and the TUI keep `queueMode: steer` and `queueMode: redirect` distinct, with dedicated `/steer` and `/redirect` commands. | Near | Medium | Add equivalent redirect discoverability to Lemon Web and any future client without collapsing it into steer. |
...
| Update, back up, migrate, recover, and uninstall the product | Hermes has update check/plan/backup, receipts, multi-profile restart/version verification, full backup/import, profile distributions, checkpoints, doctor/dump/debug/logs/status/insights, completions, and uninstall commands; see [updating](https://hermes-agent.nousresearch.com/docs/getting-started/updating). | Lemon now has a documented, versioned `~/.lemon` data contract and source/packaged `backup contract/create/list/verify/restore` lifecycle with atomic private bundles, exact checksums, credential exclusions, guarded overwrite, and rollback receipts. It also has release channels and signed checksums, source `lemon update --check`, config/skill migration, installer/uninstaller, doctor/support bundles, readiness/proofs, and hot reload. Update plan/apply/rollback receipts, fleet/profile restart, and shell completion remain absent. | Near for local backup/recovery; partial overall | **High** | Add update plan/apply/rollback receipts and shell completions, then prove recovery across a real version transition. |
...
5. **Install/update/backup** — the documented, atomic, verified user-state
   backup/restore vertical is now implemented in source and packaged CLI.
   Widen supported installation and add update plan/apply/rollback receipts.

...
| Discover and administer the product from one CLI | Hermes documents roughly sixty top-level families in its pinned [CLI reference](https://github.com/NousResearch/hermes-agent/blob/4f22543509d1b91dc45bcb369447126c5eb14fb7/website/docs/reference/cli-commands.md): chat, models/fallback/MoA, sessions, cron, profiles, skills, memory, approvals, backups, updates, logs, plugins, MCP, completions, and more. | Packaged [`LemonCli.CLI`](../../apps/lemon_cli/lib/lemon_cli/cli.ex) now has nine families: setup, model, gateway, doctor, config, secrets, channels, verified backup/restore, and a complete profile lifecycle/canonical-chat surface. Source `bin/lemon` adds node, send, media, model/provider inspection, policy, proofs, readiness, skill, usage, and update through Mix tasks. Backup and profile commands have consistent help/JSON/errors, but there is still no single generated command map or completion surface. | Partial | **High** | Promote the remaining supported runtime surfaces into the packaged CLI, then generate the reference and shell completions from one registry. |

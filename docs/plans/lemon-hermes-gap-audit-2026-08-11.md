...
| No redirect interruption | [`AbortSignal`](../../apps/lemon_agent/lib/lemon_agent/abort_signal.ex), the [streaming loop](../../apps/lemon_agent/lib/lemon_agent/loop/streaming.ex), and [`CodingAgent.Session`](../../apps/coding_agent/lib/coding_agent/session.ex) implement redirect while preserving completed tool results and degrading to steer during tool execution. | Local and named-node native runs support invocation-bound steer/redirect with authenticated destination acknowledgement and fail-closed terminal races. Control-plane chat methods and the TUI preserve redirect as a distinct mode; equivalent Web discoverability remains. |
| Provider fallback was only a preview ordering | [`ProviderRouting`](../../apps/lemon_agent/lib/lemon_agent/model_runtime/provider_routing.ex) builds route candidates; [`ProviderFallback`](../../apps/coding_agent/lib/coding_agent/session/provider_fallback.ex) executes per-turn provider and credential failover with cooldowns and session pinning. Source/package CLI and token-required Web management now share preview-first pool/fallback configuration. | Runtime resilience and normal self-service administration are implemented; provider subscription OAuth and named MoA presets remain separate gaps. |
...
| Discover and administer the product from one CLI | Hermes documents roughly sixty top-level families in its pinned [CLI reference](https://github.com/NousResearch/hermes-agent/blob/4f22543509d1b91dc45bcb369447126c5eb14fb7/website/docs/reference/cli-commands.md): chat, models/fallback/MoA, sessions, cron, profiles, skills, memory, approvals, backups, updates, logs, plugins, MCP, completions, and more. | Packaged [`LemonCli.CLI`](../../apps/lemon_cli/lib/lemon_cli/cli.ex) now covers setup, model/provider routing, gateway, doctor, config, secrets and external sources, channels, verified backup/restore, profiles/canonical chat, context references, learn-from-source, blueprints, durable session lifecycle/statistics, managed updates, and registry-generated Bash/Zsh/Fish completion. Source `bin/lemon` additionally exposes node, send, media, policy, proofs, readiness, skill, and usage through Mix tasks. | Near for daily administration; partial for broad operator families | **High** | Promote only the remaining stable source-only runtime surfaces—especially approvals, logs, MCP, and general cron—into the same help/JSON/errors/completion contract. |
...
| Create durable specialist agents and collaborate among them | [Bot Mode](https://hermes-agent.nousresearch.com/docs/user-guide/bot-mode) presents profiles as bots with independent config/model/memory/skills/SOUL, canonical chats, cron routines, DMs, groups, cross-machine roster, and remote creation. Profiles also have full [CLI lifecycle](https://hermes-agent.nousresearch.com/docs/reference/profile-commands). | Lemon now has a first-class profile record over its existing router agents and named nodes: isolated homes/workspaces, create/list/show/clone/rename/credential-safe-export/recoverable-delete, stable `agent:<id>:main` chats, named-node routing, a node-aware roster, packaged CLI/control-plane lifecycle, a TUI picker/editor, and preview-first token-required Web management. | **Near** across CLI/TUI/Web for lifecycle and canonical chat | **High** | Add profile import/restore and profile-scoped automation next; design group rooms and merged multi-controller rosters only over the shared lifecycle. |
...
| Administer a machine in the browser | Hermes's [Web Dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard) manages profiles, real TUI chat, status/resources, a large config surface, credentials, sessions, models, skills, MCP, pairing, webhooks, gateways, memory, cron, plugins, logs, and analytics. | [`LemonWeb.Router`](../../apps/lemon_web/lib/lemon_web/router.ex) now has a fail-closed token-required `/manage` shell for runtime/live-node health, durable sessions and active-run controls, provider routing, exact-confirmed skill + cron blueprints, bounded/redacted durable-memory inspection/deletion, and preview-first profile lifecycle, alongside chat and `/healthz`. Approvals, general cron, skills/MCP, logs, and broad config do not yet have equivalent Web journeys. | Partial, six bounded verticals shipped | **High** | Extend the authenticated shell one existing service at a time: approvals, general cron, skills/MCP, logs, and config. Keep each mutation scoped and browser-tested. |
| Browse, search, resume, export, archive, and safely prune sessions | Hermes [sessions](https://hermes-agent.nousresearch.com/docs/user-guide/sessions) support workspace-aware resume, rename/pin/archive/read state, FTS search, rich filtering, lineage, redacted JSONL/HTML/Markdown/QMD/trace exports, verified delete-after-export, prune, handoff, recovery, and recap. | `LemonCore.SessionLifecycle` composes the canonical stores for bounded search, title/pin/archive, redacted JSON/Markdown export, verified delete, and exact-candidate guarded prune. Web, source/package CLI, and Bun TUI now reuse the authenticated lifecycle. The TUI adds a live filterable picker, exact-key resume, digest-verified private exports, export-before-delete, non-queued destructive calls, and stable redacted errors; its authenticated Bandit proof asserts exact candidate sets and deletion/retention over isolated real session rows. Lineage, handoff, recovery, recap, read state, and richer formats remain gaps. | **Near** cross-client for the shared lifecycle | **High** | Add lineage/read state and recovery/backup semantics through the canonical store, then richer export formats where a concrete workflow needs them. |

### Agent runtime, models, tools, context, and media

| User job | Hermes at pinned upstream | Lemon at pinned baseline | Status | Priority | Concrete next closure |
| --- | --- | --- | --- | --- | --- |
| Configure models, auth pools, fallback, and per-task routing | Hermes has interactive provider/model setup, OAuth credentials, credential pools, ordered fallbacks, auxiliary task models, provider routing, custom endpoints, and selectable [Mixture of Agents](https://hermes-agent.nousresearch.com/docs/user-guide/features/mixture-of-agents) presets. The [provider catalog](https://hermes-agent.nousresearch.com/docs/integrations/providers) is user-facing. | Lemon supports a broad provider set, custom endpoints, model catalogs, per-session resolution, live fallback, credential pools/cooldowns, and route preview. Source/package `lemon providers` and token-required preview-first Web forms now inspect/edit fallback and credential-reference pools through one comment-preserving configuration boundary. | Near across runtime/CLI/Web; partial for subscription OAuth and MoA | **High** | Add only provider-specific OAuth/account ergonomics with a real need; treat MoA as a separate opt-in feature with explicit cost/budget visibility. |
| Interrupt, steer, or redirect a running agent | Hermes distinguishes hard interrupt, non-canceling steer, and model-request redirect. | Lemon implements the same semantics through router queue modes, `CodingAgent.Session`, and `LemonAgent.Loop`; named-node runs carry bounded controls to the destination and wait for authenticated acceptance. Control-plane, TUI, and Web active-run forms keep follow-up, steer, redirect, and stop distinct, preserve refused drafts, and avoid asynchronous success races. | **Parity+** for local and named-node run control | Medium | Preserve these semantics in future clients; do not collapse redirect into steer. |
...
| Attach files and reference repository context naturally | Hermes supports `@file`, `@folder`, `@git-diff`, `@url`, session references, PDF/Office/notebook extraction, and deliverable attachments; see [context references](https://hermes-agent.nousresearch.com/docs/user-guide/features/context-references) and [document extraction](https://hermes-agent.nousresearch.com/docs/user-guide/features/document-extraction). | `LemonCore.Context` and source/package `lemon context` now provide one bounded preview/resolve syntax for files, folders, git diffs, public URLs, redacted sessions, PDF, DOCX, XLSX, PPTX, notebooks, and text, with traversal/symlink/archive/SSRF/redirect defenses and explicit budgets. | Near for safe text context; partial for scanned/encrypted document fidelity and composer shortcuts | Medium | Add client composer affordances over the shared resolver and bounded OCR only behind an explicit dependency/policy decision. |
...
| User job | Hermes at pinned upstream | Lemon at pinned baseline | Status | Priority | Concrete next closure |
| --- | --- | --- | --- | --- | --- |
| Keep durable personal and project memory | Hermes combines local USER/MEMORY files, session search, background review/curation, a learning journey, and a broad set of [memory providers](https://hermes-agent.nousresearch.com/docs/user-guide/features/memory-providers). | Lemon has SQLite FTS memory, redaction-aware ingest, provider fan-out, session search, project context, memory tools, scheduled skill synthesis/curation, and a production Honcho integration. `/manage/memory` now adds bounded/redacted scope/agent/workspace-digest/kind filtering, ordinary-run and learned-source inspection, provenance/digests, and exact revision-bound single-record deletion over that same Store. Local and Honcho are the implemented providers; adaptive features remain intentionally opt-in. | Near for durable recall and safe inspection; partial for ecosystem/edit UX | Medium | Measure whether safe defaults should enable recall, add guarded edits only behind a shared review contract, and prioritize provider conformance over provider count. |
| Teach the agent from a URL, directory, or corpus and inspect what it learned | Hermes exposes `/learn`, curator flows, and the Journey learning graph/timeline. | Lemon now exposes source/packaged `lemon learn` and authenticated `learn.review`/`learn.confirm` over the existing bounded context, durable memory, deterministic skill audit, and synthesis-draft stores. Review is non-mutating and content-free; confirmation re-resolves sources and requires an exact fresh digest. `/manage/memory` renders the resulting learned-source provenance as allowlisted source types, counts, and digests without adding a second database. A full learning graph/timeline is not implemented. | Near for auditable ingestion and provenance inspection; partial for visualization | Medium | Add graph/timeline projection only if it can remain a bounded view over canonical provenance records. |
| Find, audit, install, update, bundle, and distribute skills | Hermes's [skills system](https://hermes-agent.nousresearch.com/docs/user-guide/features/skills) includes a Hub, taps, bundles, a large catalog, publishing, audit, curator, and profile distributions. | Lemon has agentskills-compatible discovery, local/project/builtin precedence, official Hermes catalog import, risk audit, trust metadata, atomic install/update, curator, model-facing management, and MCP-based discovery. Versioned unpacked bundles reuse those audits for explicit profile-local enablement, with safe catalog list/inspect/validate/preview RPC plus source/package CLI, token-required Web, and content-free Bun TUI browse/review/activate UX. Lemon still lacks a signed public registry/index, taps, publishing/export, and archive import. | Near in engine and local UX; partial in ecosystem | Medium | Add signed index/distribution and export/publish lifecycle. Do not weaken the unpacked, path-bounded activation policy for archive convenience. |
| Schedule recurring agent and script work safely | Hermes provides cron, heartbeats, recurring [loops](https://hermes-agent.nousresearch.com/docs/user-guide/features/loops), monitor jobs, no-agent scripts, chaining, blueprints, suggestions, preflight, and multiple delivery modes. | Lemon has durable cron, exact timers, jitter/retries/lineage, overlap locks, pause/resume/abort/run-now, heartbeat suppression, monitor mode, command jobs, chaining, preflight, default-model drift guard, goal continuation/auto loops, and kanban dispatch. A versioned agent-cron blueprint requires an exact plan digest, persists provenance, enables skills in the derived profile workspace, and replays without duplicate jobs through the existing scheduler; source/package CLI, token-required Web, and Bun TUI activation share one catalog service. The TUI refreshes the preview before its nonqueueable mutation and retains only bounded IDs/counts/actions/booleans/digests. | Near in engine and first local UX; partial in suggestions/templates | Medium | Add natural-language recurring-loop commands and consent-first suggestions. Expand beyond one agent cron only when a concrete workflow preserves exact confirmation and existing cron/heartbeat stores. |
...
| Keep credentials out of config and remote sandboxes | Hermes resolves credentials from 1Password, Bitwarden, or an arbitrary command and can inject them through an [egress proxy](https://hermes-agent.nousresearch.com/docs/user-guide/egress/iron-proxy) so a remote sandbox never receives the real secret. | Lemon's encrypted store now resolves through explicitly enabled, ordered [`LemonCore.Secrets.External`](../../apps/lemon_core/lib/lemon_core/secrets/external.ex) adapters for 1Password, Bitwarden Secrets Manager, and argv-only command helpers. One shared supervised runner enforces exact config, minimal child environments, stable redacted failures, timeout/output bounds, and optional bounded process-local caching; source/packaged `lemon secrets sources status|test` exposes only readiness/provenance/count metadata. Encrypted-store values still win, enabled-source failures stop before environment fallback, bootstrap credentials cannot recurse, and named nodes still receive destination-local rather than controller-resolved credentials. A general credential-injecting egress proxy for arbitrary remote/browser/tool work is not implemented. | Near for host provider credentials; partial for remote/tool egress | **High** | Design and prove an explicit outbound credential-broker boundary for remote/browser/tool workloads without passing real secrets into those execution environments. |
...
| Update, back up, migrate, recover, and uninstall the product | Hermes has update check/plan/backup, receipts, multi-profile restart/version verification, full backup/import, profile distributions, checkpoints, doctor/dump/debug/logs/status/insights, completions, and uninstall commands; see [updating](https://hermes-agent.nousresearch.com/docs/getting-started/updating). | Lemon has a versioned `~/.lemon` contract; atomic source/package backup/verify/restore; release channels; registry-driven update check/non-mutating plan/exact-confirm apply/private history/receipt-bound rollback; config/skill migration; installer/uninstaller; doctor/support bundles; readiness/proofs; and shell completion. Managed updates verify exact artifact size/checksum, confine archives, prove launcher version, and recover failed pointer flips. | Near/lead for safe local lifecycle; partial for fleet restart and publisher authentication | **High** | Add publisher signatures and exact in-flight download cancellation, then prove coordinated multi-profile restart across a real version transition. |
...
1. **Management Web vertical** — the authenticated shell, runtime/live-node
   health, sessions/run controls, provider pools/fallback, blueprints, memory,
   and profiles are implemented over shared services. Continue with approvals,
   general cron, skills/MCP, logs, and config, with real browser tests per slice.
...
3. **Profile/bot primitive** — isolated homes and capability config; create,
   clone, rename, export, recoverable delete; canonical chat; and node-aware
   roster now reach CLI/control plane/TUI/Web. Add import/restore and
   profile-scoped automation before group rooms or merged-controller rosters.
4. **Session lifecycle** — the shared service, authenticated RPC, Web,
   source/package CLI, and Bun TUI now reuse one redacted,
   verified-before-delete contract. Continue with lineage/read state/recovery
   rather than a parallel store.
5. **Install/update/backup** — documented atomic user-state backup/restore and
   managed update plan/exact apply/private history/receipt-bound rollback now
   ship in source and packaged CLI. Next add publisher signatures, exact
   in-flight download cancellation, broader platforms, and fleet restart proof.
6. **Outbound credential boundary** — supervised external 1Password,
   Bitwarden, and argv-only command sources are implemented behind the
   encrypted Lemon store. Next add credential brokerage for arbitrary
   remote/browser/tool execution without handing those environments the real
   secret.

### Medium: capability completion and polish

7. Provider subscription OAuth/account ergonomics and an opt-in cost-bounded
   MoA design; normal pool/fallback management already ships in CLI and Web.
8. Extend shipped auditable learn-from-source plus Web provenance inspection
   only with a bounded learning lineage/timeline over the canonical stores.
9. **Skill distribution** — the first versioned unpacked bundle and
   ...
   registry/audit, with source/package CLI plus Web and TUI review/activation.
   Continue with signed indexing and export/publish without adding another store.
10. **Automation templates** — the first exact-confirmed, duplicate-safe
    ...
    confirmed.
11. Composer affordances over the shipped bounded context/document resolver;
    add OCR only behind an explicit dependency and policy boundary.
12. One polished voice workflow in the primary interface.
13. Extension package lifecycle and host-health UX.
14. Multi-controller connection management and merged node/session roster UX;
    authenticated remote steer/redirect is now closed.

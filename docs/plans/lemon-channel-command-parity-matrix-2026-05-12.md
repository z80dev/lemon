...
| `/redirect &lt;correction&gt;` (also `!redirect &lt;correction&gt;`) | Telegram text command | Preview | Gated by `[gateway.telegram] allow_queue_override` (same gate as `/steer`); `UpdateProcessor.parse_queue_override/2` maps the stripped remainder to queue mode `:redirect`, which redirects the active run's in-flight model request, degrades to steer when the engine only supports steering, and falls back to a queued follow-up plus a `:notice` delivery intent when the run rejects it. A bare `/redirect` with no correction is not treated as an override. |
...
| `/redirect &lt;correction&gt;` (also `!redirect &lt;correction&gt;` text prefix) | Discord slash command plus text prefix | Preview | `Discord.SlashCommands.redirect_command_schema/0` (re-exported as `Transport.redirect_command_schema/0`) registers the command with a required `correction` option; `handle_redirect_interaction/2` and `Transport.submit_inbound_now/2`'s text-prefix parse both set queue mode `:redirect`. Ungated (Discord has no `allow_queue_override` equivalent). Redirects the active run's in-flight model request, degrades to steer, and falls back to a queued follow-up plus a `:notice` delivery intent when the run rejects it. |

...
2. Clarify memory/session-search/skills/todos in docs and prompt text.
3. Add cron parity scorecard and scheduled job prompt validation.
4. Add behavioral evals that observe actual agent traces: relevant skill → `read_skill`, prior-work prompt → `search_memory`, reusable workflow → `skill_manage`, async task receipt → `join` before final answer.

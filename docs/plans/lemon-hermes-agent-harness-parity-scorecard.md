...
Current upstream comparison authority is the
[`2026-08-30` refresh of the Hermes gap audit](lemon-hermes-gap-audit-2026-08-11.md),
pinned to official Hermes
`4f22543509d1b91dc45bcb369447126c5eb14fb7` (`v0.20.6`). Older Hermes SHAs in
this scorecard belong to the dated implementation slices that used them; they
must not be interpreted as the current competitive baseline.

## Summary

Lemon has the hard architectural primitives: supervised BEAM sessions, channel
routing, one native executor with provider/model routing, task/subagent
execution, skill registry, memory search, tool policies, approvals, and a
control plane. The scorecard below is an implementation history and regression
ledger. Current product-level setup, Web, CLI, desktop/profile, session, and
operations gaps live in the source-pinned gap audit rather than being inferred
from the age of a numbered slice.

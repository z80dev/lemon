...
- Gaps:
  - Runtime defaults still need a deliberate product decision about when to apply leaf-worker restrictions automatically.
...
1. Decide whether task-spawned internal child sessions should default to `:leaf_worker` or whether orchestrator/leaf mode should remain an explicit tool policy.
2. Continue provider-weirdness regressions for provider-normalized response error shapes beyond context length and Responses request sanitation.

...
Latest slice: Provider error bodies with deeply nested `message`, `msg`, or `reason` fields are normalized into clean provider messages when no top-level provider message exists.

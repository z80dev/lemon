...
2. Added a regression proving invalid UTF-8 inside nested function-call arguments is stripped instead of breaking Responses request construction.
...
2. Continue provider-weirdness regressions for provider-normalized response error shapes beyond context length and request sanitation.

...
- Gaps:
  ...
  - Live-model delegation now has basic parallel child/join coverage; live side-effect verification and deeper leaf/orchestrator contracts remain.
...
1. Add live-model delegation evals for leaf/orchestrator toolset restrictions.
2. Continue provider-weirdness regressions for provider-normalized response error shapes beyond context length.

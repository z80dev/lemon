...
- The final `scripts/test fast` rerun passed after tightening three
  ...
  queue-full fixture shape, and LemonAgent EventStream timeout cleanup.
...
- Focused deterministic harness tests passed:
  ...
  - LemonAgent tool-call and tool-supervision tests
...
executor and uses `LemonBrowser.LocalServer` as the BEAM-owned supervision
...
`LemonBrowser.LocalServer.status/0` reports BEAM-owned local driver state,
...
- wrap `clients/lemon-browser-node` behind an OTP-supervised browser session
  manager: started via `LemonBrowser.LocalServer`
...
- run provider-specific media workers under `LemonMedia.MediaJobSupervisor`
...
- [x] `LemonMedia.MediaJobs` records redacted job metadata with prompt hashes,
...
- [x] `LemonRouter.MediaJobRecorder` records generated final-answer
  `auto_send_files` into `LemonMedia.MediaJobs` without leaking captions, raw
  paths, or session keys
- [x] `LemonMedia.MediaJobSupervisor` and `LemonMedia.MediaJobWorker` provide an
...
- Done in preview: model-facing `media_generate_image` uses the
  `LemonMedia.MediaJobSupervisor` path with deterministic `local_svg` output and
  provider-backed `openai_image` or `vertex_imagen` output, redacted
  `LemonMedia.MediaJobs` metadata, managed SVG/PNG/JPEG/WebP artifacts, runtime
...
- Done in preview: model-facing `media_generate_speech` uses the
  `LemonMedia.MediaJobSupervisor` path with deterministic `local_wav` output and
  provider-backed `openai_tts`, `elevenlabs_tts`, or `google_tts` output,
  redacted `LemonMedia.MediaJobs` metadata, managed MP3/Opus/AAC/FLAC/WAV/PCM
...
- Done in preview: model-facing `media_transcribe_audio` uses the
  `LemonMedia.MediaJobSupervisor` path with deterministic `local_transcript`
  output and provider-backed `openai_transcribe` output, path-escape rejection,
  redacted `LemonMedia.MediaJobs` metadata, managed JSON/text transcript
  artifacts, runtime credential resolution, transient-provider retry handling,
...
- Done in preview: model-facing `media_analyze_image` uses the
  `LemonMedia.MediaJobSupervisor` path with deterministic `local_vision` output
  and provider-backed `openai_vision` output, path-escape rejection,
  local-only SVG provider restrictions, redacted `LemonMedia.MediaJobs`
...
- Done in preview: model-facing `media_generate_video` uses the
  `LemonMedia.MediaJobSupervisor` path with deterministic `local_mp4` output and
  provider-backed `openai_video` or `vertex_veo` create/poll/download output,
  redacted `LemonMedia.MediaJobs` metadata, managed MP4 artifacts, runtime
  credential resolution, transient-provider retry handling, provider-error
...
- Done in preview: opt-in provider-backed vision smoke harness via
  ...
  `LemonAgent.ModelRuntime.Credentials.provider_has_credentials?/3`, matching runtime provider
...
- Done in preview: supervised `LemonLsp.ServerManager` registry/status and

...
1. Run the direct Discord live matrix for its intended stable boundary, including
   ...
   bot-API diagnostic, external-sender inbound wait, and manual-matrix modes for exact
   prompt/reply, markdown/code, long-output, tool success/failure, and file
   delivery checks; only real external-sender inbound modes can close this blocker.
...
| Discord DM/channel/thread behavior | Hermes Discord docs define DMs, server mention rules, free-response channels, thread isolation, per-user group sessions, interrupts/concurrency, files, voice, slash commands, intents, and permissions. | Lemon has Discord adapter/support code and send-file tooling. `scripts/live_discord_matrix.py --list-channels` and `--bot-api-smoke` passed against bot `Zeebot-Debug`, guild `1475727416549969980`, and channel `general` `1475727417372049419`. `apps/lemon_channels/test/lemon_channels/adapters/discord/transport_test.exs` proves the adapter ignores self-authored messages and webhooks before routing. `scripts/live_discord_matrix.py --channel-id 1475727417372049419 --bot-token-index 0 --sender-bot-token-index 1 --manual-matrix --reset-session-between-checks --timeout 300 --result-path tmp/discord-live-proof.json` passed on 2026-05-12 and covered exact prompt/reply, markdown/code, long-output chunking, tool success/failure, and text-file attachment delivery. | Green for text-first + file delivery boundary; Partial for Hermes Discord breadth | Keep DM/thread workflows, voice, richer media, and broad slash-command parity out of stable claims unless separately implemented and proven. |
...
| Testing/evals | Hermes docs point to a large pytest suite, website tests, and RL/data-generation environments. | Lemon has `scripts/test fast/quality/clients/eval-fast/live-eval/smoke`, deterministic harness evals, product smoke, live Telegram matrix scripts, and a Discord bot-to-bot manual matrix. `scripts/test live-eval` passed against a real provider on 2026-05-12. The Discord manual matrix passed on 2026-05-12 and produced `tmp/discord-live-proof.json`. | Partial | P0 remains only for public release artifact proof. Keep deterministic, live-model, live Telegram, and live Discord suites green for release candidates. |
...
1. Keep the Telegram live matrix green for the text-first plus document-delivery
   boundary; only add rich-media/image upload proof if launch claims expand.
2. Keep the Discord live matrix green for the text-first plus file-delivery
   boundary; only add DM/thread, voice, or rich-media proof if launch claims
   expand.

...
> plus the 2026-08-12 improvement plan. Kept for historical reference. The
> install-and-setup row has one dated 2026-08-18 corrective addendum recording
> later verified release behavior; every other row remains frozen.

Last reviewed (original matrix): 2026-05-18  
Install/setup addendum: 2026-08-18
...
| --- | --- | --- | --- | --- |
| Install and setup | README advertises one-line Linux/macOS/WSL2/Termux install, native Windows beta, `hermes setup`, `hermes model`, `hermes gateway setup`, `hermes doctor`, and `hermes update`. | **2026-08-18 corrective addendum:** Lemon now has a verified one-line installer for packaged releases and a Mix-free `LemonCli.CLI` boundary in `lemon_runtime_min` and `lemon_runtime_full`. It exposes `lemon setup`, `lemon model`, `lemon gateway setup`, `lemon doctor`, `lemon config`, `lemon secrets`, and `lemon channels`; a source checkout uses the matching `./bin/lemon` commands, while `mix lemon.*` remains the contributor alternative. The installer starts `lemon setup` through `/dev/tty` by default; `--skip-setup`, no controlling terminal, and `sim_broadcast_platform` defer it. Setup derives idempotent config/secrets/provider state, creates only missing config and secrets state, skips an already usable provider, and verifies a newly configured provider unless `--skip-verify` defers its live check. The first interactive TUI launch uses the same readiness gate and does not start an unconfigured agent. Telegram and Discord gateway setup adapters are available. The release CLI returns `0`/`1`/`2` for success/failure/usage errors and exits `0` for help without running a command. The sim profile intentionally excludes this runtime CLI (except its release-specific `doctor --bundle` path). | Green — verified packaged install/setup and first-TUI gate; Partial — platform breadth and updater parity remain narrower than Hermes. | Keep the packaged scope explicit: automatic setup requires `/dev/tty`; `--skip-setup`, no TTY, and the sim profile require an explicit later setup; `--skip-verify` defers, rather than passes, a live provider check. Do not claim Windows or broad remote-updater parity from this addendum. |
| Provider/model switching | Hermes supports many hosted/custom providers, `/model`, provider routing, fallback providers, credential pools, and profile distributions. | Lemon supports multiple providers/models through `apps/lemon_ai`, config/secrets, the fixed native executor, delegated vendor task runners, live-eval env knobs, redacted provider readiness, route previews, credential-pool shape, doctor routing checks, control-plane `providers.status` live fallback proof visibility, redacted `secrets.status` store health/fallback visibility, no-value summaries for `secrets.list`, `secrets.set`, `secrets.delete`, and `secrets.exists`, control-plane `models.list` provider/capability summaries without credential values, source-wrapper `./bin/lemon models` catalog discovery, source-wrapper `./bin/lemon providers` readiness checks from the same BEAM `ProviderStatus` snapshot, source-wrapper `./bin/lemon policy` route-specific model/thinking policies, allowlisted source-wrapper `./bin/lemon secrets` secret-store tasks, and Web `/ops` live fallback proof visibility. Provider-backed live eval passed against Z.ai `glm-5-turbo` on 2026-05-12, and `scripts/live_provider_fallback_smoke.exs` passed with an intentionally invalid OpenAI primary and Z.ai fallback on 2026-05-16. | BEAM live-fallback proof plus source-wrapper model/provider/policy/secrets proof | P1. Add OAuth/custom endpoint variants and keep the fallback proof green. |
...
| Web search/fetch | Hermes has `web_search` and `web_extract` through configured backends and the Tool Gateway. | Lemon now routes search and extraction through public provider behaviours, a deterministic built-in/extension registry, capability-aware dispatch, ordered fallback, normalized results, and single-flight request coalescing. Bundled search providers are Brave, Perplexity, keyless DuckDuckGo, and SearXNG; extraction supports guarded direct fetch and Firecrawl. Provider failures retain deterministic attempt metadata, successful payloads retain the untrusted-content boundary, and redirect extraction stays behind the shared SSRF policy. The redacted `scripts/live_search_browser_leadership_smoke.exs` proof passed through a real `openai-codex:gpt-5.6-luna` session at `xhigh`, deliberately forced SearXNG readiness failure, fell back to live DuckDuckGo, returned five results from three distinct source sites, and proved multi-source synthesis without persisting prompts, answers, URLs, tool results, or credentials. | Green for the compared search/extraction scope | Keep provider contract tests and the redacted live fallback proof current. |
...
| Browser final audit | Hermes treats browser automation as a first-class tool family; Lemon release claims need current proof that the BEAM-supervised browser worker is healthy. | Lemon's browser audit now combines the existing `browser.preview` gate with `.lemon/proofs/browser-extension-smoke-latest.json` and `.lemon/proofs/search-browser-leadership-latest.json`. The extension proof uses real Chrome with the Manifest V3 extension and loopback authenticated relay, verifies explicit tab opt-in and preservation of the attached browser, and stores no relay token, port, or raw target id. The real-agent proof passed through `openai-codex:gpt-5.6-luna` at `xhigh` with three stable target-scoped reads across multiple tabs, screenshot analysis, an expected stale-target error followed by recovery, and fail-closed refusal of an unpaired controller without arbitrary evaluation or backend fallback. | Green for managed local Chrome, direct CDP, authenticated controller, and opt-in extension scope | Keep actual-Chrome and Luna proof artifacts current; hosted Browserbase/Camofox adapters remain optional provider breadth rather than a local-control parity blocker. |

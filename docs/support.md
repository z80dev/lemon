...
- preview generated-media job observability through `media.status`,
  ...
  channel message bodies, and API credentials. `LemonMedia.MediaJobSupervisor`
  owns the preview `Task.Supervisor` boundary for queued/running/completed/failed
  media job lifecycle updates and emits redacted PubSub events for operator
  surfaces.
...
through `LemonMedia.MediaJobs`; generated final-answer files that pass through

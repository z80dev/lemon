...
- ~~chat_state/chat_state_store → router move: blocked until gateway stops writing chat state — fold into Phase 2.4.~~ *Corrected 2026-08-10: premise wrong — gateway's run.ex chat-state writes fire for every run regardless of ingress; 2.4 does not remove them. The move is a standalone ~2-site question about resume-token persistence ownership (router is the only reader) and is blocked on nothing — now its own task.*

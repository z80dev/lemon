...
- `clients` follows the CI client job command order for the Bun TUI package and each Node client.
...
- Platform/cloud credentials: `TELEGRAM_BOT_TOKEN`, `DISCORD_BOT_TOKEN`, `SLACK_BOT_TOKEN`, `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`, `TWILIO_AUTH_TOKEN`, XMTP wallet keys, Feishu/DingTalk tokens.
...
cd clients/tui
bun run check
bun test test/app.test.ts test/main.test.ts test/protocol/client.test.ts
```
...
Credential preflight uses `LemonAgent.ModelRuntime.Credentials.provider_has_credentials?/3`, so it

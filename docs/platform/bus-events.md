...
| §6.4 | The gateway no longer publishes `:run_phase_changed`; `LemonRouter.PhasePublisher` is the single publisher, since the router owns the phase graph. The gateway kept a private copy of the transition validation for its logs until 2026-09, when it was removed as well: the gateway does not model the phase graph (`docs/platform/run-ownership.md`). |

...
- A module that opts into `LemonCore.Store.Table` may use generic Store operations only for its own statically declared table names. The AST-based rule resolves aliases, module attributes, and default/explicit server arities; declaring one table never exempts access to another.

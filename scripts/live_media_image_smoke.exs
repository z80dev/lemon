...
if Mix.env() == :test and
     ("--api-key-secret" in System.argv() or "vertex_imagen" in System.argv()) do
...
defmodule LemonScripts.LiveMediaImageSmoke do
  @provider "openai_image"
  @vertex_provider "vertex_imagen"
  @proof_object "lemon.media_image_smoke"
  @check_name "media_provider_openai_image"
  @vertex_check_name "media_provider_vertex_imagen"
  @default_model "gpt-image-1"
  @default_vertex_model "imagen-4.0-generate-001"
  @default_prompt "A simple flat icon of a yellow lemon on a white background. No text."
  ...
  defp live_config(opts, project_dir) do
    ...
    cond do
      System.get_env("LEMON_TEST_ALLOW_LIVE_CREDENTIALS") not in [
        ...
        {:skip, "set LEMON_TEST_ALLOW_LIVE_CREDENTIALS=1 to run media image proof"}

      provider == @provider and provider_prefixed_model?(opts[:model]) ->
        {:skip,
         {:provider_prefixed_model_not_supported_for_media_type,
          "media image proof uses the OpenAI image endpoint; pass --base-url with an unprefixed model, or use media vision proof for provider-prefixed OpenAI-compatible routing"}}
      ...
      true ->
        ...
        if credential_available?(api_key, config.providers, project_dir, provider) do
          ...
          {:skip, "no configured #{provider} credential resolved for media image proof"}
        end
    end
  end
  ...
  defp run_local_smoke(opts, project_dir) do
    ...
    artifacts_dir =
      Path.join([
        ...
        ".lemon",
        "proofs",
        "media-image-local-smoke-artifacts",
        to_string(run_id)
      ])
    ...
    params = %{
      ...
      "filename" => "live-media-image-local-smoke"
    }

    case tool.execute.("live-media-image-local-smoke", params, nil, nil) do
  ...
  defp run_smoke(config) do
    ...
    tool =
      CodingAgent.Tools.MediaGenerateImage.tool(config.project_dir,
        ...
        openai_image_base_url: if(config.provider == @provider, do: config.base_url)
      )
    ...
    params = %{
      ...
      "model" => config.model,
      "filename" => "live-media-image-smoke",
      "size" => config.size,
    ...
    case tool.execute.("live-media-image-smoke", params, nil, nil) do
      %AgentCore.Types.AgentToolResult{details: details} ->
        artifact = details["artifact"] || %{}
        artifact_path = artifact["path"]
  ...
  defp local_proof(:completed, details) do
    %{
      generated_at: DateTime.utc_now() |> DateTime.to_iso8601(),
      status: "completed",
      proof_object: "lemon.media_image_local_smoke",
      proof_scope: "media_local",
      completed_count: 1,
      skipped_count: 0,
      failed_count: 0,
      checks: [%{name: "media_local_image", status: "completed", proof_scope: "media_local"}],
      details: details,
      cleanup: cleanup()
    }
  end
  ...
  defp local_proof(:failed, details) do
    %{
      ...
      skipped_count: 0,
      failed_count: 1,
      checks: [%{name: "media_local_image", status: "failed", proof_scope: "media_local"}],
      details: details,
      cleanup: cleanup()
    }
  end
  ...
  defp default_proof_path(project_dir, true) do
    Path.join([project_dir, ".lemon", "proofs", "media-image-local-smoke-latest.json"])
  end

  defp default_proof_path(project_dir, false) do
    Path.join([project_dir, ".lemon", "proofs", "media-image-smoke-latest.json"])
  end
...
LemonScripts.LiveMediaImageSmoke.main(System.argv())

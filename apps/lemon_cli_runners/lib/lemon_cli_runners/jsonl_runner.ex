defmodule LemonCliRunners.JsonlRunner do
  ...
  def run(module, opts) do
    case start_link(module, opts) do
      {:ok, pid} ->
        stream = stream(pid)
        events = LemonAgent.EventStream.events(stream) |> Enum.to_list()
        {:ok, events}
  ...
  def init({module, opts}) do
    ...
    case acquire_session_lock(resume) do
      :ok ->
        ...
        {:ok, stream} =
          LemonAgent.EventStream.start(owner: owner, runner: self(), timeout: :infinity)
  ...
  def handle_continue(:start_subprocess, state) do
    ...
    try do
      ...
      e ->
        ...
        LemonAgent.EventStream.error(state.stream, {:spawn_failed, e})
        {:stop, :normal, state}
    end
  end
  ...
  def handle_info(:finalize_exit, %{pending_exit_code: exit_code} = state)
    ...
    LemonAgent.EventStream.complete(state.stream, [])
  ...
  def handle_info(:timeout, state) do
    ...
    LemonAgent.EventStream.error(state.stream, :timeout)
    cleanup(state)
    {:stop, :normal, state}
  end
  ...
  def handle_info({:DOWN, ref, :process, _pid, _reason}, %{owner_ref: ref} = state) do
    ...
    terminate_subprocess(state, :kill)
    LemonAgent.EventStream.cancel(state.stream, :owner_down)
    cleanup(state)
    {:stop, :normal, state}
  end
  ...
  def handle_cast({:cancel, reason}, state) do
    ...
    LemonAgent.EventStream.cancel(state.stream, reason)
          ...
          acc.resume == nil and not acc.new_session_locked ->
            case acquire_session_lock(resume) do
              ...
              {:error, :session_locked} ->
                ...
                LemonAgent.EventStream.error(acc.stream, {:session_locked, resume})
                terminate_subprocess(acc, :kill)
                {:halt, {:error, %{acc | done: true}}}
            end
  ...
  defp emit_events(events, state) do
    ...
    Enum.each(events, fn event ->
      ...
      LemonAgent.EventStream.push_async(state.stream, {:cli_event, event})
    end)
  end

defmodule LemonCliRunners.ToolActionHelpers do
  ...
  # not as `%LemonAgent.Types.AgentToolResult{...}` / `%LemonAi.Types.TextContent{...}` structs.
  def normalize_tool_result(%LemonAgent.Types.AgentToolResult{} = result) do
    result
    |> LemonAgent.get_text()
    |> String.slice(0, 200)
  end

  def normalize_tool_result(%LemonAi.Types.TextContent{text: text}) when is_binary(text),

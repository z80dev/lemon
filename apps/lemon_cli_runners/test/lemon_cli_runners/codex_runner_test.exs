...
defmodule LemonCliRunners.CodexRunnerLifecycleTest do
  ...
  defmodule MockCodexRunner do
    ...
    def build_command(_prompt, _resume, _state) do
      ...
      script = Application.get_env(:lemon_agent, :mock_codex_script, "true")
      {"bash", ["-c", script]}
    end
  ...
  setup do
    ...
    prev_codex = Application.get_env(:lemon_agent, :codex)
    prev_script = Application.get_env(:lemon_agent, :mock_codex_script)

    Application.delete_env(:lemon_agent, :codex)
    Application.delete_env(:lemon_agent, :mock_codex_script)

    on_exit(fn ->
      if prev_codex do
        Application.put_env(:lemon_agent, :codex, prev_codex)
      else
        Application.delete_env(:lemon_agent, :codex)
      end

      if prev_script do
        Application.put_env(:lemon_agent, :mock_codex_script, prev_script)
      else
        Application.delete_env(:lemon_agent, :mock_codex_script)
      end
    end)
  ...
  describe "GenServer lifecycle - start_link/1" do
    test "starts successfully with valid options" do
      ...
      Application.put_env(:lemon_agent, :mock_codex_script, script)
      ...
      events = Elixir.LemonAgent.EventStream.events(stream) |> Enum.to_list()
  ...
  describe "GenServer lifecycle - terminate" do
    test "cleans up resources on normal termination" do
      ...
      # Consume all events
      _events = Elixir.LemonAgent.EventStream.events(stream) |> Enum.to_list()

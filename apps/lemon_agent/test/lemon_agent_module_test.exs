defmodule LemonAgent.ModuleTest do
  @moduledoc """
  Comprehensive tests for the LemonAgent module's public API.
  ...
  alias LemonAgent.Types.{AgentContext, AgentTool, AgentToolResult, AgentLoopConfig}
  alias LemonAi.Types.{ImageContent, TextContent, ToolResultMessage}
  ...
  defp start_agent(opts \\ []) do
    ...
    LemonAgent.new_agent(merged_opts)
  end
  ...
  describe "start_link/1" do
    test "is an alias for new_agent/1" do
      {:ok, agent1} = LemonAgent.new_agent(model: Mocks.mock_model())
      {:ok, agent2} = LemonAgent.start_link(model: Mocks.mock_model())
    ...
    test "accepts name option" do
      ...
      {:ok, _agent} =
        LemonAgent.start_link(
          model: Mocks.mock_model(),
          name: name
        )

      state = LemonAgent.get_state(name)
      assert %LemonAgent.Types.AgentState{} = state
    end
    ...
    test "links to calling process" do
      ...
      spawn_link(fn ->
        {:ok, agent} = LemonAgent.start_link(model: Mocks.mock_model())
  ...
  describe "prompt/2" do
    test "accepts a string prompt" do
      ...
      result = LemonAgent.prompt(agent, "Hello!")

      assert result == :ok
      assert :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)
    end
    ...
    test "accepts a user message struct" do
      ...
      result = LemonAgent.prompt(agent, message)
    ...
    test "accepts a list of messages" do
      ...
      result = LemonAgent.prompt(agent, messages)
    ...
    test "returns error when already streaming" do
      ...
      :ok = LemonAgent.prompt(agent, "First")
      result = LemonAgent.prompt(agent, "Second")
    ...
    test "adds user message to conversation" do
      ...
      :ok = LemonAgent.prompt(agent, "Test prompt")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

      state = LemonAgent.get_state(agent)
      user_messages = Enum.filter(state.messages, &match?(%LemonAi.Types.UserMessage{}, &1))
  ...
  describe "continue/1" do
    test "returns error when no messages" do
      ...
      result = LemonAgent.continue(agent)
    ...
    test "returns error when last message is assistant" do
      ...
      LemonAgent.Agent.replace_messages(agent, [
    ...
    test "succeeds when last message is user" do
      ...
      LemonAgent.Agent.replace_messages(agent, [Mocks.user_message("Hello")])
    ...
    test "returns error when already streaming" do
      ...
      :ok = LemonAgent.prompt(agent, "Start")
      result = LemonAgent.continue(agent)
  ...
  describe "subscribe/2" do
    test "returns an unsubscribe function" do
      ...
      unsubscribe = LemonAgent.subscribe(agent, self())
    ...
    test "multiple subscribers receive events" do
      ...
      _unsub1 = LemonAgent.subscribe(agent, subscriber1)
      _unsub2 = LemonAgent.subscribe(agent, subscriber2)
      _unsub_self = LemonAgent.subscribe(agent, self())

      :ok = LemonAgent.prompt(agent, "Test")
  ...
  describe "wait_for_idle/2" do
    test "returns immediately when idle" do
      ...
      result = LemonAgent.wait_for_idle(agent)
    ...
    test "accepts timeout as keyword option" do
      ...
      :ok = LemonAgent.prompt(agent, "Start")
      result = LemonAgent.wait_for_idle(agent, timeout: 5000)
    ...
    test "accepts timeout as integer" do
      ...
      :ok = LemonAgent.prompt(agent, "Start")
      result = LemonAgent.wait_for_idle(agent, 5000)
    ...
    test "accepts :infinity timeout" do
      ...
      :ok = LemonAgent.prompt(agent, "Start")
      result = LemonAgent.wait_for_idle(agent, :infinity)
    ...
    test "returns error on timeout" do
      ...
      :ok = LemonAgent.prompt(agent, "Start")
      result = LemonAgent.wait_for_idle(agent, timeout: 10)

      assert result == {:error, :timeout}

      # Clean up
      assert :ok = LemonAgent.wait_for_idle(agent, timeout: 2000)
    end
  end
  ...
  describe "reset/1" do
    test "clears messages" do
      ...
      state_before = LemonAgent.get_state(agent)
      assert state_before.messages != []

      :ok = LemonAgent.reset(agent)

      state_after = LemonAgent.get_state(agent)
  ...
  describe "get_state/1" do
    ...
    test "reflects conversation messages" do
      ...
      :ok = LemonAgent.prompt(agent, "Message")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)
  ...
  describe "agent_loop/4" do
    test "returns an enumerable stream" do
      context = LemonAgent.new_context(system_prompt: "Test")
      ...
      stream = LemonAgent.agent_loop(prompts, context, config)
    ...
    test "emits agent lifecycle events" do
      ...
      events =
        prompts
        |> LemonAgent.agent_loop(context, config)
        |> Enum.to_list()
  ...
  describe "agent_loop_continue/3" do
    test "continues from existing context" do
      ...
      events =
        context
        |> LemonAgent.agent_loop_continue(config)
        |> Enum.to_list()
  ...
  describe "new_context/1" do
    ...
    test "creates context with system_prompt" do
      context = LemonAgent.new_context(system_prompt: "You are helpful")
    ...
    test "creates context with messages" do
      ...
      context = LemonAgent.new_context(messages: messages)
    ...
    test "creates context with tools" do
      tools = [Mocks.echo_tool(), Mocks.add_tool()]
      context = LemonAgent.new_context(tools: tools)
  ...
  describe "new_tool/1" do
    ...
    test "raises on missing required fields" do
      assert_raise KeyError, fn ->
        LemonAgent.new_tool(name: "incomplete")
      end

      assert_raise KeyError, fn ->
        LemonAgent.new_tool(description: "incomplete")
      end
  ...
  describe "new_tool_result/1" do
    ...
    test "creates tool result with content" do
      ...
      result = LemonAgent.new_tool_result(content: content)
    ...
    test "creates tool result with details" do
      ...
      result = LemonAgent.new_tool_result(details: details)
    ...
    test "creates tool result with trust level" do
      result = LemonAgent.new_tool_result(trust: :untrusted)
  ...
  describe "text_content/1" do
    ...
    test "creates TextContent with empty string" do
      content = LemonAgent.text_content("")
  ...
  describe "image_content/2" do
    test "creates ImageContent with data and default mime type" do
      ...
      content = LemonAgent.image_content(base64_data)
    ...
    test "creates ImageContent with custom mime type" do
      ...
      content = LemonAgent.image_content(base64_data, "image/jpeg")
    ...
    test "creates ImageContent with various mime types" do
      ...
      png = LemonAgent.image_content(base64_data, "image/png")
      jpeg = LemonAgent.image_content(base64_data, "image/jpeg")
      gif = LemonAgent.image_content(base64_data, "image/gif")
      webp = LemonAgent.image_content(base64_data, "image/webp")
  ...
  describe "get_text/1" do
    ...
    test "extracts text from content list" do
      ...
      text = LemonAgent.get_text(content)
  ...
  describe "type aliases" do
    ...
    test "tool_result type works" do
      result = LemonAgent.new_tool_result(content: [])
      assert %AgentToolResult{} = result
    end
  end
  ...
  describe "integration: full agent conversation" do
    test "complete prompt -> response flow" do
      ...
      _unsub = LemonAgent.subscribe(agent, self())

      :ok = LemonAgent.prompt(agent, "Can you help me?")
    ...
    test "complete flow with tool execution" do
      ...
      :ok = LemonAgent.prompt(agent, "Add numbers")
      ...
      :ok = LemonAgent.wait_for_idle(agent, timeout: 5000)
    ...
    test "multiple prompts in sequence" do
      ...
      :ok = LemonAgent.prompt(agent, "First prompt")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

      state1 = LemonAgent.get_state(agent)
      assert length(state1.messages) == 2

      :ok = LemonAgent.prompt(agent, "Second prompt")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

      state2 = LemonAgent.get_state(agent)
    ...
    test "reset between conversations" do
      ...
      state_after_reset = LemonAgent.get_state(agent)
      assert state_after_reset.messages == []

      :ok = LemonAgent.prompt(agent, "After reset")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

      state_final = LemonAgent.get_state(agent)
  ...
  describe "edge cases" do
    test "handles empty string prompt" do
      ...
      :ok = LemonAgent.prompt(agent, "")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

      state = LemonAgent.get_state(agent)
      user_msg = Enum.find(state.messages, &match?(%LemonAi.Types.UserMessage{}, &1))
    ...
    test "handles very long prompt" do
      ...
      :ok = LemonAgent.prompt(agent, long_prompt)
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)
    ...
    test "handles unicode in prompts" do
      ...
      :ok = LemonAgent.prompt(agent, unicode_prompt)
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)
    ...
    test "handles newlines in prompts" do
      ...
      :ok = LemonAgent.prompt(agent, multiline_prompt)
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)
    ...
    test "handles stream error gracefully" do
      ...
      :ok = LemonAgent.prompt(agent, "Will fail")
      :ok = LemonAgent.wait_for_idle(agent, timeout: 1000)

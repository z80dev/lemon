defmodule LemonAgent.TelemetryTest do
  ...
  alias LemonAgent.Types.{AgentContext, AgentLoopConfig, AgentTool}
  ...
  describe "telemetry event name consistency" do
    test "all loop telemetry events use [:lemon_agent, :loop, *] prefix", %{
      handler_id: handler_id,
      collector: collector
    } do
      # Attach to all possible loop events
      loop_events = [
        [:lemon_agent, :loop, :start],
        [:lemon_agent, :loop, :end]
      ]
      ...
      for {event_name, _measurements, _metadata} <- telemetry_events do
        assert [:lemon_agent, :loop | _rest] = event_name
      end
    end

    test "all tool_task telemetry events use [:lemon_agent, :tool_task, *] prefix", %{
      handler_id: handler_id,
      collector: collector
    } do
      tool_events = [
        [:lemon_agent, :tool_task, :start],
        [:lemon_agent, :tool_task, :end],
      ...
      # Verify all collected events have the correct prefix
      for {event_name, _measurements, _metadata} <- telemetry_events do
        assert [:lemon_agent, :tool_task | _rest] = event_name
      end
    end

    test "all tool_result telemetry events use [:lemon_agent, :tool_result, *] prefix", %{
      handler_id: handler_id,
      collector: collector
    } do
      attach_telemetry([[:lemon_agent, :tool_result, :emit]], handler_id, collector)
      ...
      for {event_name, _measurements, _metadata} <- telemetry_events do
        assert [:lemon_agent, :tool_result | _rest] = event_name
      end
    end
  end
  ...
  describe "tool task telemetry" do
    test "emits :start event when tool execution begins", %{
      ...
      attach_telemetry([[:lemon_agent, :tool_task, :start]], handler_id, collector)
      ...
      assert event_name == [:lemon_agent, :tool_task, :start]
    ...
    test "emits :end event when tool execution completes", %{
      ...
      attach_telemetry([[:lemon_agent, :tool_task, :end]], handler_id, collector)
      ...
      {event_name, measurements, metadata} = hd(matching_events)
      assert event_name == [:lemon_agent, :tool_task, :end]
      assert Map.has_key?(measurements, :system_time)
      assert metadata.tool_name == "echo"
    ...
    test "emits :end with is_error=true when tool raises exception", %{
      ...
      end_events =
        Enum.filter(telemetry_events, fn {name, _, meta} ->
          name == [:lemon_agent, :tool_task, :end] and meta.tool_call_id == "call_crash_test"
        end)
    ...
    test "emits telemetry for multiple parallel tool calls", %{
      ...
      start_events =
        Enum.filter(telemetry_events, fn {name, _, meta} ->
          name == [:lemon_agent, :tool_task, :start] and
            meta.tool_call_id in ["call_parallel_1", "call_parallel_2"]
        end)

      end_events =
        Enum.filter(telemetry_events, fn {name, _, meta} ->
          name == [:lemon_agent, :tool_task, :end] and
            meta.tool_call_id in ["call_parallel_1", "call_parallel_2"]
        end)
    ...
    test "emits :emit event with trust=:trusted for successful tool results", %{
      ...
      matching_events =
        Enum.filter(telemetry_events, fn {name, _measurements, metadata} ->
          name == [:lemon_agent, :tool_result, :emit] and
      ...
      {event_name, measurements, metadata} = hd(matching_events)
      assert event_name == [:lemon_agent, :tool_result, :emit]
      assert Map.has_key?(measurements, :system_time)
      assert metadata.tool_name == "echo"
      ...
      untrusted_tool = %AgentTool{
        ...
        execute: fn _id, %{"text" => text}, _signal, _on_update ->
          %LemonAgent.Types.AgentToolResult{
            content: [%LemonAi.Types.TextContent{type: :text, text: text}],
            trust: :untrusted
          }
        end
      }
  ...
  describe "loop telemetry" do
    test "emits :start event when loop begins", %{handler_id: handler_id, collector: collector} do
      attach_telemetry([[:lemon_agent, :loop, :start]], handler_id, collector)
      ...
      assert event_name == [:lemon_agent, :loop, :start]
      assert Map.has_key?(measurements, :system_time)
    ...
    test "emits :end event with duration when loop completes", %{
      ...
      attach_telemetry([[:lemon_agent, :loop, :end]], handler_id, collector)
      ...
      {event_name, measurements, metadata} = hd(telemetry_events)
      assert event_name == [:lemon_agent, :loop, :end]
      assert Map.has_key?(measurements, :system_time)
    ...
    test "tool_task :end with is_error=true when tool raises exception", %{
      ...
      end_events =
        Enum.filter(telemetry_events, fn {name, _m, meta} ->
          name == [:lemon_agent, :tool_task, :end] and meta.tool_call_id == "crash_error_test"
        end)
    ...
    test "telemetry events are emitted even for no tool calls", %{
      ...
      start_events =
        Enum.filter(telemetry_events, fn {name, _, _} ->
          name == [:lemon_agent, :loop, :start]
        end)

      end_events =
        Enum.filter(telemetry_events, fn {name, _, _} ->
          name == [:lemon_agent, :loop, :end]
        end)
  ...
  describe "telemetry handler management" do
    test "multiple handlers can attach to same events", %{
      ...
      attach_telemetry([[:lemon_agent, :loop, :start]], handler_id, collector)
      attach_telemetry([[:lemon_agent, :loop, :start]], handler_id2, collector2)

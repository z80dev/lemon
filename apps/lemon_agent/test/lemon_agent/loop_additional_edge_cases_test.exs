defmodule LemonAgent.LoopAdditionalEdgeCasesTest do
  @moduledoc """
  Additional edge case tests for LemonAgent.Loop.
  ...
  describe "stream event edge cases" do
    test "handles stream that emits no events before completion" do
      ...
      stream_fn = fn _model, _context, _options ->
        ...
        Task.start(fn ->
          ...
          LemonAi.EventStream.complete(stream, empty_response)
        end)
    ...
    test "handles stream that only emits start but never done" do
      ...
      stream_fn = fn _model, _context, _options ->
        ...
        Task.start(fn ->
          ...
          LemonAi.EventStream.push(stream, {:start, partial_response})
          Process.sleep(10)
          LemonAi.EventStream.complete(stream, partial_response)
        end)
    ...
    test "handles stream with error event mid-stream" do
      ...
      stream_fn = fn _model, _context, _options ->
        ...
        Task.start(fn ->
          LemonAi.EventStream.push(stream, {:start, partial_response})
          LemonAi.EventStream.push(stream, {:text_start, 0, partial_response})
          LemonAi.EventStream.push(stream, {:error, :rate_limited, partial_response})
          LemonAi.EventStream.complete(stream, partial_response)
        end)
  ...
  describe "telemetry" do
    ...
    test "emits loop start and end telemetry" do
      ...
      assert_receive {:telemetry, [:lemon_agent, :loop, :start], %{system_time: _},
      ...
      assert_receive {:telemetry, [:lemon_agent, :loop, :end],
    ...
    test "emits tool task telemetry" do
      ...
      {tool_start_measurements, tool_start_meta} =
        assert_receive_tool_task_telemetry([:lemon_agent, :tool_task, :start], "call_telemetry")
      ...
      {tool_end_measurements, tool_end_meta} =
        assert_receive_tool_task_telemetry([:lemon_agent, :tool_task, :end], "call_telemetry")
    ...
    test "emits tool task end telemetry on crash with is_error true" do
      ...
      {_tool_end_measurements, tool_end_meta} =
        assert_receive_tool_task_telemetry([:lemon_agent, :tool_task, :end], "call_crash_tele")

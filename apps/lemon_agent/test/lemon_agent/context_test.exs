defmodule LemonAgent.ContextTest do
  ...
  describe "estimate_size/2" do
    ...
    test "emits telemetry event", %{handler_id: handler_id, collector: collector} do
      attach_telemetry([[:lemon_agent, :context, :size]], handler_id, collector)
      ...
      assert event_name == [:lemon_agent, :context, :size]
  ...
  describe "check_size/3" do
    ...
    test "emits telemetry for warning", %{handler_id: handler_id, collector: collector} do
      attach_telemetry([[:lemon_agent, :context, :warning]], handler_id, collector)
      ...
      {event_name, measurements, metadata} = hd(events)
      assert event_name == [:lemon_agent, :context, :warning]
  ...
  describe "truncate/2" do
    ...
    test "emits telemetry when truncating", %{handler_id: handler_id, collector: collector} do
      ...
      if dropped > 0 do
        ...
        {event_name, measurements, metadata} = hd(events)
        assert event_name == [:lemon_agent, :context, :truncated]
  ...
  describe "telemetry emission validation" do
    ...
    test "check_size emits warning telemetry with correct level", %{
      ...
      warning_events =
        Enum.filter(events, fn {name, _, _} -> name == [:lemon_agent, :context, :warning] end)
    ...
    test "truncate emits telemetry when messages are dropped", %{
      ...
      if dropped > 0 do
        truncated_events =
          Enum.filter(events, fn {name, _, _} -> name == [:lemon_agent, :context, :truncated] end)
    ...
    test "multiple estimate_size calls emit multiple telemetry events", %{
      ...
      size_events =
        Enum.filter(events, fn {name, _, _} -> name == [:lemon_agent, :context, :size] end)

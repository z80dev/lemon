defmodule LemonAgent.EventStream do
  @moduledoc """
  ...
  def handle_info({:DOWN, ref, :process, _pid, reason}, %{runner_ref: ref} = state) do
    ...
    Logger.warning("LemonAgent.EventStream runner died: #{inspect(reason)}, emitting error")
  ...
  def handle_info({:DOWN, ref, :process, pid, reason}, %{task_pid: pid, task_ref: ref} = state) do
    if reason not in [:normal, :shutdown] and not state.done and not state.canceled do
      Logger.warning("LemonAgent.EventStream task crashed: #{inspect(reason)}")
    end
  ...
  def handle_info(:stream_timeout, state) do
    Logger.debug("LemonAgent.EventStream timeout, canceling stream")
  ...
  defp handle_overflow(_event, %{drop_strategy: :error} = state, log_on_overflow) do
    if log_on_overflow do
      Logger.warning("LemonAgent.EventStream dropping event due to queue overflow")
    end
      ...
      {:dictionary, dictionary} ->
        case List.keyfind(dictionary, :agent_abort_signal, 0) do
          {:agent_abort_signal, abort_ref} when is_reference(abort_ref) ->
            LemonAgent.AbortSignal.abort(abort_ref)
  ...
  defp task_shutdown_grace_ms do
    case Application.get_env(:lemon_agent, :event_stream_cancel_grace_ms) do

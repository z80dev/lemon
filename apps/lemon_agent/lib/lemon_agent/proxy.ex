defmodule LemonAgent.Proxy do
  ...
  alias LemonAi.Types.{AssistantMessage, TextContent, ThinkingContent, ToolCall, Usage, Cost}
  ...
  """
  @spec stream_proxy(LemonAi.Types.Model.t(), LemonAi.Types.Context.t(), ProxyStreamOptions.t()) ::
          EventStream.t()
  ...
  defp encode_message(%LemonAi.Types.UserMessage{} = msg) do
    %{
      role: "user",
  ...
  defp encode_message(%LemonAi.Types.AssistantMessage{} = msg) do
    %{
      role: "assistant",
  ...
  defp encode_message(%LemonAi.Types.ToolResultMessage{} = msg) do
    %{
      role: "tool_result",
  ...
  defp encode_content_block(%LemonAi.Types.TextContent{} = c) do
    %{type: "text", text: c.text, textSignature: c.text_signature}
  end

  defp encode_content_block(%LemonAi.Types.ThinkingContent{} = c) do
    %{type: "thinking", thinking: c.thinking, thinkingSignature: c.thinking_signature}
  end

  defp encode_content_block(%LemonAi.Types.ImageContent{} = c) do
    %{type: "image", data: c.data, mimeType: c.mime_type}
  end

  defp encode_content_block(%LemonAi.Types.ToolCall{} = c) do
    %{
      type: "toolCall",
      id: c.id,
      name: c.name,

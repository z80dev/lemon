defmodule LemonAgent.TextGeneration do
  @moduledoc """
  Lightweight text completion bridge for apps that depend on `agent_core`.

  Keeps direct `LemonAi.*` calls inside `agent_core`, which lets callers stay
  ...
  def complete_text(provider, model_id, prompt, opts \\ [])
    ...
    model_registry = Keyword.get(opts, :model_registry, LemonAi.Models)
    context_module = Keyword.get(opts, :context_module, LemonAi.Types.Context)
    ai_module = Keyword.get(opts, :ai_module, LemonAi)

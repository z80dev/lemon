defmodule LemonAgent.SubagentSupervisor do
  @moduledoc """
  ...
  def start_subagent(opts) do
    ...
    agent_opts =
      cond do
        registry_key != nil ->
          Keyword.put(agent_opts, :name, LemonAgent.AgentRegistry.via(registry_key))
    ...
    child_spec = %{
      id: make_ref(),
      start: {LemonAgent.Agent, :start_link, [agent_opts]},
      restart: :temporary,
      shutdown: 5_000,
      type: :worker
    }
  ...
  def stop_subagent_by_key(key) do
    case LemonAgent.AgentRegistry.lookup(key) do

defmodule LemonAgent do
  ...
  - `LemonAgent.Agent` - GenServer for stateful agent management
  - `LemonAgent.Loop` - Core agentic loop implementation
  - `LemonAgent.EventStream` - Async event producer/consumer
  - `LemonAgent.Types` - Type definitions and structs
  - `LemonAgent.Proxy` - Stream proxy for event transformation
  """

  alias LemonAgent.Types.{AgentContext, AgentTool, AgentToolResult}
  ...
  def new_agent(opts) do
    LemonAgent.Agent.start_link(normalize_agent_opts(opts))
  end
  ...
  @spec prompt(
          agent(),
          String.t() | LemonAgent.Types.agent_message() | list(LemonAgent.Types.agent_message())
        ) ::
          :ok | {:error, term()}
  defdelegate prompt(agent, message), to: LemonAgent.Agent
  ...
  defdelegate continue(agent), to: LemonAgent.Agent
  ...
  """
  @spec abort(agent()) :: :ok
  defdelegate abort(agent), to: LemonAgent.Agent
  ...
  defdelegate subscribe(agent, subscriber), to: LemonAgent.Agent
  ...
  defdelegate wait_for_idle(agent, timeout \\ :infinity), to: LemonAgent.Agent

  @doc """
  Reset the agent state, clearing all messages.
  """
  @spec reset(agent()) :: :ok
  defdelegate reset(agent), to: LemonAgent.Agent
  ...
  defdelegate get_state(agent), to: LemonAgent.Agent
  ...
  """
  @spec agent_loop([LemonAgent.Types.agent_message()], context(), config(), function() | nil) ::
          Enumerable.t()
  def agent_loop(prompts, context, config, stream_fn \\ nil) do
    LemonAgent.Loop.stream(prompts, context, config, stream_fn)
  end
  ...
  def agent_loop_continue(context, config, stream_fn \\ nil) do
    LemonAgent.Loop.stream_continue(context, config, stream_fn)
  end
  ...
  def text_content(text) when is_binary(text) do
    %LemonAi.Types.TextContent{text: text}
  end
  ...
  def image_content(data, mime_type \\ "image/png") do
    %LemonAi.Types.ImageContent{data: data, mime_type: mime_type}
  end
  ...
  """
  @spec get_text(tool_result() | [LemonAi.Types.TextContent.t() | LemonAi.Types.ImageContent.t()]) ::
          String.t()
  def get_text(%AgentToolResult{content: content}), do: get_text(content)

  def get_text(content) when is_list(content) do
    content
    |> Enum.filter(&match?(%LemonAi.Types.TextContent{}, &1))
    |> Enum.map(& &1.text)
    |> Enum.join("")
  end
end

defmodule LemonMCP.Server do
  ...
  """
  @spec get_initialize_result(GenServer.server()) :: Protocol.InitializeResult.t()
  ...
  def handle_call(:get_initialize_result, _from, state) do
    caps_map =
      state.capabilities
      |> Protocol.ServerCapabilities.new()
      |> Protocol.capabilities_to_map()
  ...
  defp build_server_opts(opts) do
    %{
      ...
      capabilities: Keyword.get(opts, :capabilities, [tools: true])
    }
  end

  defp build_capabilities(opts) do
    base_caps = [tools: true]
    additional_caps = Keyword.get(opts, :capabilities, [])
    Keyword.merge(base_caps, additional_caps)
  end
  ...
  defp do_call_tool(state, name, _arguments) do

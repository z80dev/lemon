defmodule Mix.Tasks.Lemon.Onboard.GeminiTest do
  use ExUnit.Case, async: false

  import ExUnit.CaptureIO

  alias LemonCore.Secrets
  alias Mix.Tasks.Lemon.Onboard.Gemini

  setup do
    tmp_dir =
      Path.join(
        System.tmp_dir!(),
        "lemon_onboard_gemini_#{System.unique_integer([:positive])}"
      )
  ...
  test "stores token payload and updates providers config", %{tmp_dir: tmp_dir} do
    ...
    assert output =~ "Google Gemini CLI onboarding complete."

    assert {:ok, ~s({"token":"gemini-token-123","projectId":"proj-123"})} =
             Secrets.get("llm_google_gemini_cli_api_key",
               prefer_env: false,
               env_fallback: false
             )
  ...
  test "sets defaults when --set-default is enabled", %{tmp_dir: tmp_dir} do
    ...
    assert get_in(config_map, ["defaults", "provider"]) == "google_gemini_cli"
    assert get_in(config_map, ["defaults", "model"]) == "google_gemini_cli:gemini-2.5-pro"
  end

defmodule LemonAi.CallDispatcherEdgeTest do
  @moduledoc """
  ...
  alias LemonAi.CallDispatcher
  alias LemonAi.RateLimiter
  ...
  describe "telemetry events" do
    test "emits dispatch event on successful dispatch", %{provider: provider} do
      ...
      assert_receive {:telemetry, [:lemon_ai, :dispatcher, :dispatch], measurements, metadata},
                     1000
    ...
    test "emits rejected event when circuit is open", %{provider: provider} do
      ...
      assert_receive {:telemetry, [:lemon_ai, :dispatcher, :rejected], measurements, metadata},
                     1000
    ...
    test "telemetry events include accurate timing information", %{provider: provider} do
      ...
      :telemetry.attach_many(
        ...
        [[:lemon_ai, :dispatcher, :dispatch], [:lemon_ai, :dispatcher, :rejected]],
        fn event, measurements, metadata, _config ->
          send(parent, {:telemetry, event, measurements, metadata})
        end,
        nil
      ...
      assert_receive {:telemetry, [:lemon_ai, :dispatcher, :dispatch], measurements, _metadata},
                     1000

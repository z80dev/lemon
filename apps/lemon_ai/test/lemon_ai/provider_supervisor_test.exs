defmodule LemonAi.ProviderSupervisorTest do
  @moduledoc """
  Tests for LemonAi.ProviderSupervisor dynamic supervisor.
  ...
  alias LemonAi.ProviderSupervisor
  ...
  defp cleanup_child(pid) when is_pid(pid) do
    if Process.alive?(pid) do
      DynamicSupervisor.terminate_child(LemonAi.ProviderSupervisor, pid)
    end
  end
  ...
  setup do
    ...
    case Process.whereis(LemonAi.ProviderSupervisor) do
      nil ->
  ...
  describe "supervisor startup" do
    ...
    test "supervisor has correct module" do
      pid = Process.whereis(LemonAi.ProviderSupervisor)
      assert {:registered_name, LemonAi.ProviderSupervisor} = Process.info(pid, :registered_name)
    end
  ...
  describe "child process management" do
    ...
    test "can terminate a child process" do
      ...
      :ok = DynamicSupervisor.terminate_child(LemonAi.ProviderSupervisor, pid)
  ...
  describe "dynamic provider addition" do
    test "can add provider services dynamically" do
      ...
      {:ok, pid} = DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, child_spec)
    ...
    test "adding same provider twice returns already_started" do
      ...
      {:ok, pid1} = DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, child_spec)

      # Trying to start again with same name should fail
      result = DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, child_spec)
  ...
  describe "error isolation between providers" do
    ...
    test "supervisor survives child crash" do
      sup_pid = Process.whereis(LemonAi.ProviderSupervisor)
      name = unique_name()
      ...
      assert Process.whereis(LemonAi.ProviderSupervisor) == sup_pid
    ...
    test "child crash does not propagate to supervisor" do
      ...
      assert Process.alive?(Process.whereis(LemonAi.ProviderSupervisor))
    end
    ...
    test "exception in child init does not crash supervisor" do
      ...
      {:error, :intentional_crash} =
        DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, child_spec)
  ...
  describe "integration with ProviderRegistry" do
    ...
    test "can start RateLimiter under supervisor" do
      ...
      {:ok, pid} = LemonAi.RateLimiter.ensure_started(provider)
    ...
    test "CircuitBreaker and RateLimiter for same provider coexist" do
      ...
      {:ok, cb_pid} = LemonAi.CircuitBreaker.ensure_started(provider)
      {:ok, rl_pid} = LemonAi.RateLimiter.ensure_started(provider)
    ...
    test "provider services survive registry operations" do
      ...
      LemonAi.ProviderRegistry.register(provider, TestModule)
      assert LemonAi.ProviderRegistry.registered?(provider)

      LemonAi.ProviderRegistry.unregister(provider)
      refute LemonAi.ProviderRegistry.registered?(provider)
      ...
      tasks =
        for _ <- 1..20 do
          Task.async(fn ->
            name = unique_name()
            {:ok, pid} = start_test_worker(name)
            _ = DynamicSupervisor.which_children(LemonAi.ProviderSupervisor)
            _ = DynamicSupervisor.count_children(LemonAi.ProviderSupervisor)
            cleanup_child(pid)
            :ok
          end)
        end
  ...
  describe "edge cases and error handling" do
    test "terminating non-existent child returns error" do
      ...
      result = DynamicSupervisor.terminate_child(LemonAi.ProviderSupervisor, fake_pid)
    ...
    test "supervisor handles empty child spec gracefully" do
      ...
      result =
        try do
          DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, %{})
        rescue
          e -> {:error, e}
        end

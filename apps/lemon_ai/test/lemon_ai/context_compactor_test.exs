defmodule LemonAi.ContextCompactorTest do
  @moduledoc """
  Tests for the LemonAi.ContextCompactor module.
  """

  use ExUnit.Case, async: true

  alias LemonAi.ContextCompactor
  alias LemonAi.Types.{Context, AssistantMessage, TextContent}
  ...
  describe "telemetry" do
    test "emits compaction_started event" do
      ref =
        :telemetry_test.attach_event_handlers(self(), [
          [:lemon_ai, :context_compactor, :compaction_started]
        ])

      context = build_test_context(5)
      ContextCompactor.compact(context, strategy: :truncation)

      assert_received {[:lemon_ai, :context_compactor, :compaction_started], ^ref, _measurements,
                       metadata}
    ...
    test "emits compaction_succeeded event on success" do
      ref =
        :telemetry_test.attach_event_handlers(self(), [
          [:lemon_ai, :context_compactor, :compaction_succeeded]
        ])

      context = build_test_context(5)
      ContextCompactor.compact(context, strategy: :truncation)

      assert_received {[:lemon_ai, :context_compactor, :compaction_succeeded], ^ref,
    ...
    test "emits compaction_failed event on failure" do
      ref =
        :telemetry_test.attach_event_handlers(self(), [
          [:lemon_ai, :context_compactor, :compaction_failed]
        ])

      context = Context.new() |> Context.add_user_message("Hello")
      ContextCompactor.compact(context, strategy: :truncation)

      assert_received {[:lemon_ai, :context_compactor, :compaction_failed], ^ref, _measurements,
                       metadata}

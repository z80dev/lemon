defmodule AiTest do
  ...
  alias LemonAi.ProviderRegistry
  ...
  describe "get_text/1" do
    test "extracts text from assistant message" do
      ...
      assert LemonAi.get_text(message) == "Hello world!"
    end
    ...
    test "returns empty string for no text content" do
      ...
      assert LemonAi.get_text(message) == ""
    end
  end
  ...
  describe "get_thinking/1" do
    test "extracts thinking from assistant message" do
      message = %AssistantMessage{
        content: [
          %LemonAi.Types.ThinkingContent{thinking: "Step one."},
          %LemonAi.Types.ThinkingContent{thinking: "Step two."}
        ],
        api: :test,
        provider: :test,
        model: "test",
      ...
      assert LemonAi.get_thinking(message) == "Step one.Step two."
    end
    ...
    test "returns empty string for no thinking content" do
      ...
      assert LemonAi.get_thinking(message) == ""
    end
  end

  describe "get_tool_calls/1" do
    test "extracts tool calls from assistant message" do
      tool_call = %LemonAi.Types.ToolCall{id: "call_1", name: "tool_a", arguments: %{"a" => 1}}
      ...
      assert LemonAi.get_tool_calls(message) == [tool_call]
    end
  end
  ...
  describe "calculate_cost/2" do
    test "calculates cost correctly" do
      ...
      cost = LemonAi.calculate_cost(model, usage)
  ...
  describe "stream options normalization" do
    test "accepts keyword list options and preserves defaults" do
      ...
      {:ok, stream} = LemonAi.stream(model, context, temperature: 0.5)
    ...
    test "propagates stream_timeout to providers" do
      ...
      {:ok, stream} = LemonAi.stream(model, context, %{stream_timeout: 12_345})
  ...
  describe "dispatcher integration" do
    test "returns rate_limited when dispatcher blocks the request" do
      ...
      start_supervised!(
        {LemonAi.RateLimiter, provider: provider, tokens_per_second: 0, max_tokens: 1}
      )

      start_supervised!({LemonAi.CircuitBreaker, provider: provider, failure_threshold: 5})

      # Consume the single token so the next request is rate-limited.
      assert :ok = LemonAi.RateLimiter.acquire(provider)
      ...
      assert {:error, :rate_limited} = LemonAi.stream(model, context)
  ...
  describe "unknown api handling" do
    test "returns unknown_api error for unregistered providers" do
      ...
      assert {:error, {:unknown_api, :unknown_api}} = LemonAi.stream(model, context)
    end
  end
  ...
  describe "complete/3" do
    test "returns final message from provider stream" do
      ...
      assert {:ok, %AssistantMessage{} = message} = LemonAi.complete(model, context)
      assert LemonAi.get_text(message) == "ok"
    end
  end
end

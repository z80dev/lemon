defmodule LemonAi.Providers.OpenAICompletionsTest do
  ...
  test "ZAI requests can disable reasoning and require a tool call" do
    ...
    tool = %LemonAi.Types.Tool{name: "act", description: "Act", parameters: %{"type" => "object"}}
  ...
  test "includes tools: [] when tool history exists but no tools provided" do
    ...
    assistant = %LemonAi.Types.AssistantMessage{
      role: :assistant,
      content: [tool_call],
      api: :openai_completions,
      provider: :openai,
      model: "gpt-4o-mini",
      usage: %LemonAi.Types.Usage{cost: %LemonAi.Types.Cost{}},
      stop_reason: :stop,
  ...
  test "thinking blocks are converted to text when required" do
    ...
    assistant =
      %LemonAi.Types.AssistantMessage{
        role: :assistant,
        content: [
          %LemonAi.Types.ThinkingContent{thinking: "thoughts"},

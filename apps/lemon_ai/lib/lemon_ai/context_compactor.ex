defmodule LemonAi.ContextCompactor do
  @moduledoc """
  ...
  defp build_truncated_messages(preserved, summary) do
    ...
    summary_message = %LemonAi.Types.UserMessage{
  ...
  defp message_token_estimate(%LemonAi.Types.UserMessage{content: content})
       when is_binary(content) do
    LemonAi.Tokens.estimate_chars(content) + 4
  end
  ...
  defp message_token_estimate(%LemonAi.Types.UserMessage{content: content})
    ...
    content
    |> Enum.map(fn
      %{text: text} when is_binary(text) -> LemonAi.Tokens.estimate_chars(text)
      _ -> 10
    end)
    |> Enum.sum()
    |> Kernel.+(4)
  ...
  defp message_token_estimate(%LemonAi.Types.AssistantMessage{content: content}) do
    content
    |> Enum.map(fn
      %{text: text} when is_binary(text) -> LemonAi.Tokens.estimate_chars(text)
      %{thinking: thinking} when is_binary(thinking) -> LemonAi.Tokens.estimate_chars(thinking)
      _ -> 10
    end)
    |> Enum.sum()
    |> Kernel.+(4)
  ...
  defp message_token_estimate(%LemonAi.Types.ToolResultMessage{content: content}) do
    content
    |> Enum.map(fn
      %{text: text} when is_binary(text) -> LemonAi.Tokens.estimate_chars(text)
      _ -> 10
  ...
  defp emit_telemetry(event, metadata) do
    :telemetry.execute(
      [:lemon_ai, :context_compactor, event],

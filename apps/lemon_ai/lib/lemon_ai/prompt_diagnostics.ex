defmodule LemonAi.PromptDiagnostics do
  @moduledoc """
  ...
  alias LemonAi.Types.{AssistantMessage, Context, Model, StreamOptions}
  ...
  defp record_introspection(data, %StreamOptions{} = opts) do
    ...
    :telemetry.execute(
      [:lemon_ai, :prompt_diagnostics, :llm_call],
  ...
  defp message_bytes(%LemonAi.Types.UserMessage{content: content}), do: content_bytes(content)

  defp message_bytes(%LemonAi.Types.ToolResultMessage{content: content}),
    do: content_bytes(content)

  defp message_bytes(%LemonAi.Types.AssistantMessage{content: content}),
    do: content_bytes(content)
  ...
  defp content_block_bytes(%LemonAi.Types.TextContent{text: text}), do: byte_size(text)

  defp content_block_bytes(%LemonAi.Types.ThinkingContent{thinking: thinking}),
    do: byte_size(thinking)

  defp content_block_bytes(%LemonAi.Types.ImageContent{data: data}), do: byte_size(data)

  defp content_block_bytes(%LemonAi.Types.ToolCall{name: name, id: id, arguments: args}) do
    args_bytes =
  ...
  defp approx_tokens_from_bytes(bytes) when is_integer(bytes) and bytes >= 0 do
    ...
    LemonAi.Tokens.estimate_byte_count(bytes + 3)
  end

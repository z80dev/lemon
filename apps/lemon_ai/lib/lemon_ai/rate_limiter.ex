defmodule LemonAi.RateLimiter do
  @moduledoc """
  ...
  ## Usage
      ...
      LemonAi.RateLimiter.release(:anthropic)
  ...
  defp via_tuple(provider) do
    {:via, Registry, {LemonAi.RateLimiterRegistry, provider}}
  end

  defp start_provider_child(opts) do
    if Process.whereis(LemonAi.ProviderSupervisor) do
      provider = Keyword.fetch!(opts, :provider)
      ...
      case DynamicSupervisor.start_child(LemonAi.ProviderSupervisor, child_spec) do
        {:ok, pid} ->
          {:ok, pid}
  ...
  defp default_tokens_per_second do
    Application.get_env(:lemon_ai, :rate_limiter, [])

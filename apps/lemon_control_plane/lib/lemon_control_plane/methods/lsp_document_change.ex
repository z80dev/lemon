defmodule LemonControlPlane.Methods.LspDocumentChange do
  @moduledoc """
  Handler for `lsp.document.change`.
  """

  @behaviour LemonControlPlane.Method

  @impl true
  def name, do: "lsp.document.change"
  ...
  def handle(params, _ctx) do
    ...
    with {:ok, session_id} <- required(params, "sessionId", :missing_session_id),
      ...
      {:error, error} ->
        {:error, {:internal_error, "Failed to change LSP document", inspect(error)}}
    end
  end

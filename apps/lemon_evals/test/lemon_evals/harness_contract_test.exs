defmodule LemonEvals.HarnessContractTest do
  ...
  describe "run/1" do
    test "includes Hermes-style memory and skill contract checks", %{tmp_dir: tmp_dir} do
      ...
      assert "dedicated_tool_preference_contract" in names
      ...
      assert "agent_loop_skill_refinement_trace_contract" in names
      assert "agent_loop_memory_trace_contract" in names
      assert "agent_loop_workspace_memory_file_contract" in names
      assert "agent_loop_workspace_memory_update_contract" in names
    ...
    test "live-model checks are opt in", %{tmp_dir: tmp_dir} do
      ...
      refute "live_model_memory_topic_contract" in default_names
      refute "live_model_workspace_memory_file_contract" in default_names
      ...
      refute "live_model_coding_repair_contract" in default_names
      ...
      assert "live_model_memory_topic_contract" in live_names
      assert "live_model_workspace_memory_file_contract" in live_names
      ...
      assert "live_model_coding_repair_contract" in live_names
    end
    ...
    test "live-model memory topic eval reports missing credentials without provider access", %{
      tmp_dir: tmp_dir
    } do
      result = Harness.live_model_memory_topic_contract_eval(tmp_dir, live_api_key: "")

      assert result.name == "live_model_memory_topic_contract"
      assert result.status == :fail
      assert result.details.reason =~ "LEMON_EVAL_API_KEY"
    end

    test "live-model workspace memory file eval reports missing credentials without provider access",
         %{
           tmp_dir: tmp_dir
         } do
      result = Harness.live_model_workspace_memory_file_contract_eval(tmp_dir, live_api_key: "")

      assert result.name == "live_model_workspace_memory_file_contract"
      assert result.status == :fail
      assert result.details.reason =~ "LEMON_EVAL_API_KEY"
    end
    ...
    test "live-model relevant skill usage eval reports missing credentials without provider access",
         %{
           tmp_dir: tmp_dir
         } do
      result = Harness.live_model_relevant_skill_usage_contract_eval(tmp_dir, live_api_key: "")

      assert result.name == "live_model_relevant_skill_usage_contract"
      assert result.status == :fail
      assert result.details.reason =~ "LEMON_EVAL_API_KEY"
    end
    ...
    test "live-model coding repair eval reports missing credentials without provider access",
         %{
           tmp_dir: tmp_dir
         } do
      result = Harness.live_model_coding_repair_contract_eval(tmp_dir, live_api_key: "")

      assert result.name == "live_model_coding_repair_contract"
      assert result.status == :fail
      assert result.details.reason =~ "LEMON_EVAL_API_KEY"
    end
  end

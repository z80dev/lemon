...
- LemonSim credential helpers should resolve OAuth-backed secrets through `LemonAiRuntime.Auth.OAuthSecretResolver`, not `Ai.Auth.OAuthSecretResolver`. This matters for Gemini CLI, Antigravity, Copilot, and other providers whose stored secret payload is not itself the final runtime token format.

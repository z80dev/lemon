  ...
  def default_opts(overrides \\ []) when is_list(overrides) do
    ...
    model =
      Keyword.get_lazy(overrides, :model, fn ->
        Config.resolve_configured_model!(config, "supply chain",
          error_label: "Supply Chain example"
        )
      end)

    stream_options =
      Keyword.get_lazy(overrides, :stream_options, fn ->
        %{api_key: Config.resolve_provider_api_key!(model.provider, config, "supply chain")}
      end)

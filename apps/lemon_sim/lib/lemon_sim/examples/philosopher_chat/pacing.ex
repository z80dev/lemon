defmodule LemonSim.Examples.PhilosopherChat.Pacing do
  ...
  def mentioned?(world, agent_id) do
    case get(world, :messages, []) |> List.last() do
      ...
      last ->
        text = get(last, :text, "") |> to_string() |> String.downcase()
        name = agent_name(agent_id) |> String.downcase()
        first_name = name |> String.split(" ") |> List.first()

        text != "" and
          (String.contains?(text, first_name) or String.contains?(text, agent_id))
    end
  end
  ...
  def pick_responder(world, rng_state) do
    ...
    pool =
      if length(pool) > 1 and is_binary(last_author) and last_author != @user_id do
  ...
  def continue_chatter?(pace, rng_state) do
    {hit, next_rng} = chance?(rng_state, continue_chance(pace))
    {hit, next_rng}
  end
  ...
  def idle_chatter?(pace, rng_state) do
    {hit, next_rng} = chance?(rng_state, idle_chance(pace))
    {hit, next_rng}
  end
  ...
  def recent_agent_count(world, now_ms) do
    get(world, :messages, [])
    |> Enum.filter(fn m ->
      get(m, :author) != @user_id and get(m, :at_ms, 0) >= now_ms - 3_600_000
    end)
    |> length()
  end

defmodule LemonSim.Examples.Skirmish.VideoGenerator do
  ...
  @doc """
  Generates a gameplay video from a skirmish JSONL log file.
  ...
  ## Options
    ...
    * `:hold_frames` - Duplicate frames per step for pacing (default: #{@default_hold_frames})
  ...
  @doc """
  Checks that required external tools (`rsvg-convert` and `ffmpeg`) are available
  on the system PATH.

  Returns `:ok` or `{:error, {:missing_tools, list}}`.
  """
  ...
  defp create_temp_dir do
    dir_name = "lemon_replay_#{System.system_time(:millisecond)}"

defmodule LemonSim.Examples.Courtroom.VideoGenerator do
  @moduledoc false

  alias LemonSim.Examples.Courtroom.{FrameRenderer, GameLog}
  ...
  @doc """
  Generates a gameplay video from a courtroom trial JSONL log file.
  ...
  defp create_temp_dir do
    dir_name = "lemon_courtroom_replay_#{System.system_time(:millisecond)}"
  ...
  defp hold_count_for(entry, base_hold) do
    ...
    multiplier =
      cond do
        type == "init" -> 3
        type == "verdict" -> 5
        has_event?(events, "verdict_reached") -> 4
        has_event?(events, "phase_changed") -> 2
        has_event?(events, "objection_ruled") -> 2
        true -> 1
      end

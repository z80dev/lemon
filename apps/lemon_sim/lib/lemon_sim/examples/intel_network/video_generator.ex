defmodule LemonSim.Examples.IntelNetwork.VideoGenerator do
  @moduledoc false

  alias LemonSim.Examples.IntelNetwork.{FrameRenderer, GameLog}
  ...
  @doc """
  Generates a gameplay video from an intel_network JSONL log file.
  ...
  defp create_temp_dir do
    dir_name = "lemon_intel_network_replay_#{System.system_time(:millisecond)}"
  ...
  defp hold_count_for(entry, base_hold) do
    ...
    multiplier =
      cond do
        type == "init" -> 3
        type == "game_over" -> 5
        has_event?(events, "intel_leaked") -> 4
        has_event?(events, "round_advanced") -> 2
        has_event?(events, "agent_framed") -> 3

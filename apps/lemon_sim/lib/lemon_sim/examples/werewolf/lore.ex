defmodule LemonSim.Examples.Werewolf.Lore do
  @moduledoc """
  Generates rich character profiles for werewolf players via LLM.
  ...
  defp build_prompt(players, backstory_connections) do
    sorted_player_ids =
      players
      |> Map.keys()
      |> Enum.sort()
    ...
    player_descriptions =
      ...
      |> Enum.map(fn {id, info} ->
        ...
        "- #{id}: role=#{role}, traits=[#{Enum.join(traits, ", ")}]#{if connections != "", do: ", connections=[#{connections}]", else: ""}"
      end)
      |> Enum.join("\n")
    ...
    """
    Generate rich character profiles for a Werewolf village game. Each player needs a vivid, \
    memorable backstory that makes them feel like a real person in a small medieval village.

    Players:
    #{player_descriptions}

    For each player, generate a JSON object with these fields:
    - "full_name": An expanded full name (e.g., "Alice Thornberry")
    - "occupation": A village trade (blacksmith, herbalist, innkeeper, woodcutter, weaver, etc.)
    ...
    - "backstory": 2-3 sentence narrative incorporating their connections

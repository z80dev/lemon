defmodule LemonSim.Examples.Auction.VideoGenerator do
  @moduledoc false

  alias LemonSim.Examples.Auction.{FrameRenderer, GameLog}
  ...
  @doc """
  Generates a gameplay video from an auction JSONL log file.
  ...
  defp create_temp_dir do
    dir_name = "lemon_auction_replay_#{System.system_time(:millisecond)}"
  ...
  defp hold_count_for(entry, base_hold) do
    ...
    multiplier =
      cond do
        type == "init" -> 4
        type == "game_over" -> 6
        has_event?(events, "item_won") -> 3
        has_event?(events, "item_unsold") -> 2
        has_event?(events, "auction_started") -> 2
        has_event?(events, "round_started") -> 2
        has_event?(events, "scoring_complete") -> 4
        true -> 1
      end

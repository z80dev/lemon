defmodule Mix.Tasks.Lemon.Sim.AuctionReplay do
  @shortdoc "Generates a video replay from an Auction House game log"
  @moduledoc """
  Renders an Auction House game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.auction_replay path/to/auction.jsonl [options]
  ...
  alias LemonSim.Examples.Auction.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.auction_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Auction House replay from #{log_path}...")

defmodule Mix.Tasks.Lemon.Sim.LegislatureReplay do
  @shortdoc "Generates a video replay from a Legislature game log"
  @moduledoc """
  Renders a Legislature game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.legislature_replay path/to/legislature.jsonl [options]
  ...
  alias LemonSim.Examples.Legislature.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.legislature_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Legislature replay from #{log_path}...")

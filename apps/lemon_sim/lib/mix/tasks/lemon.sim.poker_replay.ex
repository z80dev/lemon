defmodule Mix.Tasks.Lemon.Sim.PokerReplay do
  @shortdoc "Generates a video replay from a Poker game log"
  @moduledoc """
  Renders a Poker (no-limit hold'em) game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.poker_replay path/to/poker.jsonl [options]
  ...
  alias LemonSim.Examples.Poker.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.poker_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Poker replay from #{log_path}...")

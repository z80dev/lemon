defmodule Mix.Tasks.Lemon.Sim.DungeonCrawlReplay do
  @shortdoc "Generates a video replay from a Dungeon Crawl game log"
  @moduledoc """
  Renders a Dungeon Crawl game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.dungeon_crawl_replay path/to/dungeon_crawl.jsonl [options]
  ...
  alias LemonSim.Examples.DungeonCrawl.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.dungeon_crawl_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Dungeon Crawl replay from #{log_path}...")
    gen_opts = []

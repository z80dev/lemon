defmodule Mix.Tasks.Lemon.Sim.CourtroomReplay do
  @shortdoc "Generates a video replay from a Courtroom Trial game log"
  @moduledoc """
  Renders a Courtroom Trial transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.courtroom_replay path/to/courtroom.jsonl [options]
  ...
  alias LemonSim.Examples.Courtroom.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.courtroom_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Courtroom Trial replay from #{log_path}...")

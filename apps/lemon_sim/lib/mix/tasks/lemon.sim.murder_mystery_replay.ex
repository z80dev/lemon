defmodule Mix.Tasks.Lemon.Sim.MurderMysteryReplay do
  @shortdoc "Generates a video replay from a Murder Mystery game log"
  @moduledoc """
  Renders a Murder Mystery game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.murder_mystery_replay path/to/murder_mystery.jsonl [options]
  ...
  alias LemonSim.Examples.MurderMystery.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.murder_mystery_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Murder Mystery replay from #{log_path}...")

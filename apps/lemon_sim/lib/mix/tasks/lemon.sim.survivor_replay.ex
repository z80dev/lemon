defmodule Mix.Tasks.Lemon.Sim.SurvivorReplay do
  @shortdoc "Generates a video replay from a Survivor game log"
  @moduledoc """
  Renders a Survivor game transcript (JSONL) into an MP4 video.

  ## Usage

      mix lemon.sim.survivor_replay path/to/survivor.jsonl [options]
  ...
  alias LemonSim.Examples.Survivor.VideoGenerator
  ...
  def run(args) do
    ...
    cond do
      ...
      positional == [] ->
        Mix.shell().error("Usage: mix lemon.sim.survivor_replay <path.jsonl> [options]")
  ...
  defp do_generate(log_path, opts) do
    Mix.shell().info("Generating Survivor replay from #{log_path}...")
    gen_opts = []

...
- Every portable slash command must pass the same configured guild/channel
  allowlists and `deny_unbound_channels` binding check as ordinary messages and
  queue/steer commands before reading session state or invoking the agent runtime.

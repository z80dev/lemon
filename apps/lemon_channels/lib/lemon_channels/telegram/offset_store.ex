defmodule LemonChannels.Telegram.OffsetStore do
  @moduledoc false
  ...
  def get(account_id, token) do
    if store_available?() do
      ...
      case LemonCore.Store.get(@table, key) do
  ...
  def put(account_id, token, offset) when is_integer(offset) do
    if store_available?() do
      key = key(account_id, token)
      _ = LemonCore.Store.put(@table, key, offset)
    end
  ...
  defp store_available? do
    Code.ensure_loaded?(LemonCore.Store) and
      function_exported?(LemonCore.Store, :get, 2) and

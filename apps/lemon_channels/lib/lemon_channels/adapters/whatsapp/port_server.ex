defmodule LemonChannels.Adapters.WhatsApp.PortServer do
  ...
  def handle_cast({:command, %{} = command}, %{port: port} = state) when is_port(port) do
    ...
    case write_command(port, command) do
      ...
      {:error, reason} ->
        emit_error(state, "failed to write command to whatsapp bridge", %{reason: inspect(reason)})
        {:noreply, state}
    end
  end
  ...
  def handle_cast({:command, %{} = command}, state) do
    ...
    emit_error(state, "whatsapp bridge unavailable", %{
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    Logger.warning("whatsapp bridge exited (status=#{status})")

    emit_error(state, "whatsapp bridge exited", %{status: status})
  ...
  defp maybe_open_port(state) do
    with {:ok, node_path} <- node_path(),
      ...
      {:error, reason} ->
        emit_error(state, "failed to start whatsapp bridge", %{reason: reason})
  ...
  defp default_script_path do
    candidates = [
      safe_app_dir(:lemon_channels, "priv/whatsapp_bridge.mjs"),
      safe_app_dir(:lemon_gateway, "priv/whatsapp_bridge.mjs"),
      Path.expand("../../../../../lemon_gateway/priv/whatsapp_bridge.mjs", __DIR__)
    ]

    Enum.find(candidates, fn path -> is_binary(path) and File.exists?(path) end) ||
      safe_app_dir(:lemon_gateway, "priv/whatsapp_bridge.mjs") ||
      Path.expand("apps/lemon_gateway/priv/whatsapp_bridge.mjs", File.cwd!())
  end
  ...
  defp handle_line(line, state) when is_binary(line) do
    case Jason.decode(line) do
      ...
      {:ok, other} ->
        emit_error(state, "whatsapp bridge emitted non-object JSON", %{payload: other})
        state

      {:error, _reason} ->
        emit_error(state, "whatsapp bridge emitted invalid JSON", %{line: line})
        state
    end
  end

  defp send_event(pid, event) when is_pid(pid), do: send(pid, {:whatsapp_bridge_event, event})
  ...
  defp replay_connect_command(%{port: port, connect_command: %{} = command} = state)
    ...
    case write_command(port, command) do
      ...
      {:error, reason} ->
        emit_error(state, "failed to replay whatsapp connect command", %{reason: inspect(reason)})

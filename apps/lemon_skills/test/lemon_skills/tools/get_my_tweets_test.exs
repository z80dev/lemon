defmodule LemonSkills.Tools.GetMyTweetsTest do
  use ExUnit.Case, async: false

  alias AgentCore.Types.AgentToolResult
  alias Ai.Types.TextContent
  alias LemonSkills.Tools.GetMyTweets
  ...
  test "returns not configured error when X API is unavailable" do
    assert %AgentToolResult{
           ...
           } = GetMyTweets.execute("call-1", %{}, nil, nil)
  ...
  test "validates limit parameter type" do
    assert %AgentToolResult{
           ...
           } = GetMyTweets.execute("call-2", %{"limit" => "abc"}, nil, nil)
  end

  test "validates limit parameter value" do
    assert %AgentToolResult{
             details: %{error: "Parameter 'limit' must be a positive integer"}
           } = GetMyTweets.execute("call-3", %{"limit" => 0}, nil, nil)
  end

  test "tool/0 returns a valid tool definition with pagination support" do
    tool = GetMyTweets.tool()

    assert tool.name == "get_my_tweets"
    assert tool.label == "Get My Tweets"
    assert is_function(tool.execute, 4)
    assert tool.parameters["properties"]["limit"]
    assert tool.parameters["properties"]["exclude_replies"]
    assert tool.parameters["properties"]["exclude_retweets"]
    assert tool.parameters["properties"]["pagination_token"]
    assert tool.parameters["properties"]["pagination_token"]["type"] == "string"
  end
end

defmodule LemonCore.Doctor.RegistrationTest do
  ...
  describe "check registration" do
    test "built-ins stand on their own, independent of registrations" do
      # The umbrella registers LemonAutomation.Doctor.Checks.Synthesis in
      # config/config.exs; running lemon_core on its own leaves that module
      # unloadable, so the registered list is empty there. Either way the
      # registry only ever yields loadable modules and never duplicates a
      # built-in.

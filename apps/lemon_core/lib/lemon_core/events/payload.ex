defmodule LemonCore.Events.Payload do
  ...
  A payload module declares its event type and fields; this macro supplies the struct, the
  strict constructor publishers use, and the lenient constructor that coerces a payload
  arriving from outside this node.
  ...
  For one cycle these structs implemented `Access` so that publishers could migrate ahead of
  their consumers; that shim is gone. Consumers pattern-match the struct or read a field, and
  anything holding a payload that may not be a struct yet — a legacy map over the wire, an
  injected control-plane event — coerces once at its entry point with
  `LemonCore.Events.coerce/2`. See `docs/platform/bus-events.md` §4.3.
  ...
  - `from_map/1` — lenient: drops unknown keys, accepts string keys, passes structs through.
    The coercion `LemonCore.Events.coerce/2` and `cast/2` call for you.
  """

defmodule LemonCore.RouterBridge do
  ...
  def handle_inbound(msg) do
    case call(:router, :handle_inbound, [msg]) do
      {:error, _} = error -> error
      _ -> :ok
    end
  end
  ...
  def abort_session(session_key, reason \\ :user_requested) when is_binary(session_key) do
    case call(:router, :abort, [session_key, reason]) do
      {:error, _} = error -> error
      _ -> :ok
    end
  end
  ...
  def abort_run(run_id, reason \\ :user_requested) when is_binary(run_id) do
    case call(:router, :abort_run, [run_id, reason]) do
      {:error, _} = error -> error
      _ -> :ok
    end
  end
  ...
  def keep_run_alive(run_id, decision \\ :continue)
    ...
    case call(:router, :keep_run_alive, [run_id, decision]) do
      {:error, _} = error -> error
      _ -> :ok
    end
  end

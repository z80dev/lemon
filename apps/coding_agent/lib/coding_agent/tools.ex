defmodule CodingAgent.Tools do
  @moduledoc """
  Tool registry and factory functions for coding agent tools.

  Provides pre-configured tool sets for different use cases:
  - `coding_tools/2` - Full access tools (read, read_skill, skill_manage, memory_topic, memory, search_memory, session_search, checkpoint, write, edit, hashline_edit, patch, lsp_diagnostics, bash, grep, find, ls, webfetch, websearch, browser tools, media_status, media_generate_image, media_generate_speech, media_transcribe_audio, media_analyze_image, media_generate_video, todo, kanban, task, agent, parent_question, tool_auth, extensions_status) plus any runtime-registered tools
  - `read_only_tools/2` - Exploration tools (read, read_skill, search_memory, session_search, grep, find, ls)
  - `all_tools/2` - All available tools as a map
  """

  alias CodingAgent.Tools.{
    Agent,
    Read,
    ReadSkill,
    SkillManage,
    SessionSearch,
    Checkpoint,
    Write,
    Edit,
    HashlineEdit,
    Patch,
    LspDiagnostics,
    Bash,
    Grep,
    Find,
    Ls,
    WebFetch,
    WebSearch,
    BrowserNavigate,
    BrowserTabs,
    BrowserTabOpen,
    BrowserTabActivate,
    BrowserTabClose,
    BrowserSnapshot,
    BrowserGetContent,
    BrowserClick,
    BrowserType,
    BrowserHover,
    BrowserSelectOption,
    BrowserUploadFile,
    BrowserDownload,
    BrowserPress,
    BrowserScroll,
    BrowserBack,
    BrowserWaitForSelector,
    BrowserEvaluate,
    BrowserEvents,
    BrowserGetCookies,
    BrowserSetCookies,
    BrowserClearState,
    BrowserScreenshot,
    BrowserAnalyze,
    BrowserExec,
    ComputerUse,
    Todo,
    Truncate,
    Task,
    ToolAuth,
    ExtensionsStatus,
    ParentQuestion
  }

  alias LemonSkills.Tools.{
    MemoryTopic,
    Memory,
    SearchMemory,
    MediaStatus,
    MediaGenerateImage,
    MediaGenerateSpeech,
    MediaTranscribeAudio,
    MediaAnalyzeImage,
    MediaGenerateVideo,
    Kanban
  }

  @doc """
  Get the default coding tools (read, read_skill, skill_manage, memory_topic, memory, search_memory, session_search, checkpoint, write, edit, hashline_edit, patch, lsp_diagnostics, bash, grep, find, ls, webfetch, websearch, browser tools, media_status, media_generate_image, media_generate_speech, media_transcribe_audio, media_analyze_image, media_generate_video, todo, kanban, task, agent, parent_question, tool_auth, extensions_status) plus any runtime-registered tools.

  ## Options
  - Any options are passed through to individual tools
  """
  @spec coding_tools(String.t(), keyword()) :: [LemonAgent.Types.AgentTool.t()]
  def coding_tools(cwd, opts \\ []) do
    [
      Read.tool(cwd, opts),
      ReadSkill.tool(cwd, opts),
      SkillManage.tool(cwd, opts),
      MemoryTopic.tool(cwd, opts),
      Memory.tool(cwd, opts),
      SearchMemory.tool(cwd, opts),
      SessionSearch.tool(cwd, opts),
      Checkpoint.tool(cwd, opts),
      Write.tool(cwd, opts),
      Edit.tool(cwd, opts),
      HashlineEdit.tool(cwd, opts),
      Patch.tool(cwd, opts),
      LspDiagnostics.tool(cwd, opts),
      Bash.tool(cwd, opts),
      Grep.tool(cwd, opts),
      Find.tool(cwd, opts),
      Ls.tool(cwd, opts),
      WebFetch.tool(cwd, opts),
      WebSearch.tool(cwd, opts),
      BrowserNavigate.tool(cwd, opts),
      BrowserTabs.tool(cwd, opts),
      BrowserTabOpen.tool(cwd, opts),
      BrowserTabActivate.tool(cwd, opts),
      BrowserTabClose.tool(cwd, opts),
      BrowserSnapshot.tool(cwd, opts),
      BrowserGetContent.tool(cwd, opts),
      BrowserClick.tool(cwd, opts),
      BrowserType.tool(cwd, opts),
      BrowserHover.tool(cwd, opts),
      BrowserSelectOption.tool(cwd, opts),
      BrowserUploadFile.tool(cwd, opts),
      BrowserDownload.tool(cwd, opts),
      BrowserPress.tool(cwd, opts),
      BrowserScroll.tool(cwd, opts),
      BrowserBack.tool(cwd, opts),
      BrowserWaitForSelector.tool(cwd, opts),
      BrowserEvaluate.tool(cwd, opts),
      BrowserEvents.tool(cwd, opts),
      BrowserGetCookies.tool(cwd, opts),
      BrowserSetCookies.tool(cwd, opts),
      BrowserClearState.tool(cwd, opts),
      BrowserScreenshot.tool(cwd, opts),
      BrowserAnalyze.tool(cwd, opts),
      BrowserExec.tool(cwd, opts),
      ComputerUse.tool(cwd, opts),
      MediaStatus.tool(cwd, opts),
      MediaGenerateImage.tool(cwd, opts),
      MediaGenerateSpeech.tool(cwd, opts),
      MediaTranscribeAudio.tool(cwd, opts),
      MediaAnalyzeImage.tool(cwd, opts),
      MediaGenerateVideo.tool(cwd, opts),
      Todo.tool(cwd, opts),
      Kanban.tool(cwd, opts),
      Task.tool(cwd, opts),
      Agent.tool(cwd, opts),
      ParentQuestion.tool(cwd, opts),
      ToolAuth.tool(cwd, opts),
      ExtensionsStatus.tool(cwd, opts)
    ]
    |> then(fn builtins -> builtins ++ registered_tools(builtins, cwd, opts) end)
  end

  # Tools contributed at runtime by apps the platform does not know about.
  # Built-ins win on a name collision (`LemonAgent.ToolRegistry`), so anything
  # already in `builtins` is dropped rather than shadowing or duplicating it.
  defp registered_tools(builtins, cwd, opts) do
    taken = MapSet.new(builtins, & &1.name)

    LemonAgent.ToolRegistry.all()
    |> Enum.reject(fn {name, _module} -> MapSet.member?(taken, Atom.to_string(name)) end)
    |> Enum.map(fn {_name, module} -> module.tool(cwd, opts) end)
  end

  @doc """
  Get read-only tools for exploration (read, read_skill, search_memory, session_search, grep, find, ls).
  """
  @spec read_only_tools(String.t(), keyword()) :: [LemonAgent.Types.AgentTool.t()]
  def read_only_tools(cwd, opts \\ []) do
    [
      Read.tool(cwd, opts),
      ReadSkill.tool(cwd, opts),
      SearchMemory.tool(cwd, opts),
      SessionSearch.tool(cwd, opts),
      Grep.tool(cwd, opts),
      Find.tool(cwd, opts),
      Ls.tool(cwd, opts)
    ]
  end

  @doc """
  Get all available tools as a map keyed by name.
  """
  @spec all_tools(String.t(), keyword()) :: %{String.t() => LemonAgent.Types.AgentTool.t()}
  def all_tools(cwd, opts \\ []) do
    %{
      "read" => Read.tool(cwd, opts),
      "read_skill" => ReadSkill.tool(cwd, opts),
      "skill_manage" => SkillManage.tool(cwd, opts),
      "memory_topic" => MemoryTopic.tool(cwd, opts),
      "memory" => Memory.tool(cwd, opts),
      "search_memory" => SearchMemory.tool(cwd, opts),
      "session_search" => SessionSearch.tool(cwd, opts),
      "checkpoint" => Checkpoint.tool(cwd, opts),
      "write" => Write.tool(cwd, opts),
      "edit" => Edit.tool(cwd, opts),
      "hashline_edit" => HashlineEdit.tool(cwd, opts),
      "patch" => Patch.tool(cwd, opts),
      "lsp_diagnostics" => LspDiagnostics.tool(cwd, opts),
      "bash" => Bash.tool(cwd, opts),
      "grep" => Grep.tool(cwd, opts),
      "find" => Find.tool(cwd, opts),
      "ls" => Ls.tool(cwd, opts),
      "webfetch" => WebFetch.tool(cwd, opts),
      "websearch" => WebSearch.tool(cwd, opts),
      "browser_navigate" => BrowserNavigate.tool(cwd, opts),
      "browser_tabs" => BrowserTabs.tool(cwd, opts),
      "browser_tab_open" => BrowserTabOpen.tool(cwd, opts),
      "browser_tab_activate" => BrowserTabActivate.tool(cwd, opts),
      "browser_tab_close" => BrowserTabClose.tool(cwd, opts),
      "browser_snapshot" => BrowserSnapshot.tool(cwd, opts),
      "browser_get_content" => BrowserGetContent.tool(cwd, opts),
      "browser_click" => BrowserClick.tool(cwd, opts),
      "browser_type" => BrowserType.tool(cwd, opts),
      "browser_hover" => BrowserHover.tool(cwd, opts),
      "browser_select_option" => BrowserSelectOption.tool(cwd, opts),
      "browser_upload_file" => BrowserUploadFile.tool(cwd, opts),
      "browser_download" => BrowserDownload.tool(cwd, opts),
      "browser_press" => BrowserPress.tool(cwd, opts),
      "browser_scroll" => BrowserScroll.tool(cwd, opts),
      "browser_back" => BrowserBack.tool(cwd, opts),
      "browser_wait_for_selector" => BrowserWaitForSelector.tool(cwd, opts),
      "browser_evaluate" => BrowserEvaluate.tool(cwd, opts),
      "browser_events" => BrowserEvents.tool(cwd, opts),
      "browser_get_cookies" => BrowserGetCookies.tool(cwd, opts),
      "browser_set_cookies" => BrowserSetCookies.tool(cwd, opts),
      "browser_clear_state" => BrowserClearState.tool(cwd, opts),
      "browser_screenshot" => BrowserScreenshot.tool(cwd, opts),
      "browser_analyze" => BrowserAnalyze.tool(cwd, opts),
      "browser_exec" => BrowserExec.tool(cwd, opts),
      "computer_use" => ComputerUse.tool(cwd, opts),
      "media_status" => MediaStatus.tool(cwd, opts),
      "media_generate_image" => MediaGenerateImage.tool(cwd, opts),
      "media_generate_speech" => MediaGenerateSpeech.tool(cwd, opts),
      "media_transcribe_audio" => MediaTranscribeAudio.tool(cwd, opts),
      "media_analyze_image" => MediaAnalyzeImage.tool(cwd, opts),
      "media_generate_video" => MediaGenerateVideo.tool(cwd, opts),
      "todo" => Todo.tool(cwd, opts),
      "kanban" => Kanban.tool(cwd, opts),
      "truncate" => Truncate.tool(opts),
      "task" => Task.tool(cwd, opts),
      "agent" => Agent.tool(cwd, opts),
      "parent_question" => ParentQuestion.tool(cwd, opts),
      "tool_auth" => ToolAuth.tool(cwd, opts),
      "extensions_status" => ExtensionsStatus.tool(cwd, opts)
    }
    |> then(fn builtins ->
      # Built-ins last: a registration colliding with a platform tool name is
      # ignored, never swapped in. `get_tool/3` resolves through this map, so
      # merging the other way would let a satellite replace the tool the model
      # was shown the schema for.
      Map.merge(registered_tools_by_name(cwd, opts), builtins)
    end)
  end

  defp registered_tools_by_name(cwd, opts) do
    Map.new(LemonAgent.ToolRegistry.all(), fn {name, module} ->
      {Atom.to_string(name), module.tool(cwd, opts)}
    end)
  end

  @doc """
  Get a specific tool by name.
  """
  @spec get_tool(String.t(), String.t(), keyword()) :: LemonAgent.Types.AgentTool.t() | nil
  def get_tool(name, cwd, opts \\ []) do
    Map.get(all_tools(cwd, opts), name)
  end

  @doc """
  Get tools by a list of names.
  """
  @spec get_tools([String.t()], String.t(), keyword()) :: [LemonAgent.Types.AgentTool.t()]
  def get_tools(names, cwd, opts \\ []) do
    all = all_tools(cwd, opts)
    Enum.map(names, &Map.get(all, &1)) |> Enum.reject(&is_nil/1)
  end
end

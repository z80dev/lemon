defmodule CodingAgent.ToolPolicyTest do
  ...
  describe "from_profile/1" do
    ...
    test "read_only only allows read tools" do
      ...
      assert ToolPolicy.allowed?(policy, "session_search")
    ...
    test "minimal_core allows core tools and excludes redundant ones" do
      ...
      assert ToolPolicy.allowed?(policy, "task")
      assert ToolPolicy.allowed?(policy, "agent")
      assert ToolPolicy.allowed?(policy, "browser_hover")
      assert ToolPolicy.allowed?(policy, "browser_select_option")
      assert ToolPolicy.allowed?(policy, "browser_upload_file")
      assert ToolPolicy.allowed?(policy, "browser_download")
      assert ToolPolicy.allowed?(policy, "browser_wait_for_selector")
      assert ToolPolicy.allowed?(policy, "browser_evaluate")
      assert ToolPolicy.allowed?(policy, "browser_events")
      assert ToolPolicy.allowed?(policy, "browser_analyze")
      assert ToolPolicy.allowed?(policy, "media_status")
      assert ToolPolicy.allowed?(policy, "media_generate_image")
      assert ToolPolicy.allowed?(policy, "media_generate_speech")
      assert ToolPolicy.allowed?(policy, "media_transcribe_audio")
    ...
    test "safe_mode denies dangerous tools" do
      ...
      assert ToolPolicy.allowed?(policy, "browser_hover")
      assert ToolPolicy.allowed?(policy, "browser_wait_for_selector")
      refute ToolPolicy.allowed?(policy, "browser_select_option")
      refute ToolPolicy.allowed?(policy, "browser_upload_file")
      refute ToolPolicy.allowed?(policy, "browser_download")
    ...
    test "no_external denies external tools" do
      ...
      refute ToolPolicy.allowed?(policy, "browser_hover")
      refute ToolPolicy.allowed?(policy, "browser_select_option")
      refute ToolPolicy.allowed?(policy, "browser_upload_file")
      refute ToolPolicy.allowed?(policy, "browser_download")
      refute ToolPolicy.allowed?(policy, "browser_wait_for_selector")
      refute ToolPolicy.allowed?(policy, "browser_evaluate")

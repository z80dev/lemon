...
The tool catalogue (default set, additional tools, per-tool contracts for
`execute_code`, the browser tools, `memory`, `session_search`, the media
tools, checkpoints, `lsp_diagnostics` and `kanban`) lives in `README.md`
("Built-in Tools"). `CodingAgent.Tools.coding_tools/2` is the executable
list; `CodingAgent.ToolRegistry` merges extension and satellite tools into it.

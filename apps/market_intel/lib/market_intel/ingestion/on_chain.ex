defmodule MarketIntel.Ingestion.OnChain do
  ...
  # Contract addresses
  @zeebot_token "0x14d2ced95039eef74952cd1c1b129bad68bb0b07"
  ...
  def get_large_transfers do
    MarketIntel.Cache.get(:zeebot_large_transfers)
  end
  ...
  defp do_fetch(last_block) do
    ...
    Enum.each(results, fn
      {:ok, key, data} -> MarketIntel.Cache.put(key, data)
      {:error, reason} -> Logger.warning("[MarketIntel] On-chain fetch failed: #{inspect(reason)}")
    end)
  ...
  defp fetch_gas_prices do
    ...
    case HTTPoison.get("#{@base_rpc}", [], timeout: 10_000) do
      {:ok, %{status_code: 200, body: body}} ->
        # Parse gas price from response
        {:ok, :base_network_stats, %{
          gas_price_gwei: parse_gas_price(body),
          congestion: :medium,  # Could calculate from gas prices
  ...
  defp fetch_token_transfers(from_block) do
    ...
    case HTTPoison.get(url, [], timeout: 15_000) do
      {:ok, %{status_code: 200, body: body}} ->
        ...
        # Filter for large transfers (> 0.1% of supply)
  ...
  defp large_transfer?(transfer) do
    # Define threshold for "large" transfer
    # e.g., > 1M tokens or > $1000
    value = String.to_integer(transfer.value || "0")
    value > 1_000_000_000_000_000_000_000_000  # 1M tokens (18 decimals)
  end

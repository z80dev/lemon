...
- `LemonMedia.MediaJobs` — the job record store: `record/2` for a single
  ...
  retention.
- `LemonMedia.MediaJobSupervisor` — a `Task.Supervisor` facade with `start_job/2`,
  which records the job as `:queued`, starts a temporary task for it and
  returns immediately, and `status/0`, which answers with a zeroed shape rather
  than raising when the supervisor is not running.
- `LemonMedia.MediaJobWorker` — a stateless runner that takes one job through
...
- Replaced the one-shot worker GenServer and generic dynamic supervisor with
  OTP's purpose-built `Task`/`Task.Supervisor` primitives. Returned PIDs,
  cancellation, lifecycle broadcasts, failure isolation, status counts, and
  temporary no-restart behavior are unchanged.

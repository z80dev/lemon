defmodule LemonSimUi.Live.Components.BoardComponentsTest do
  ...
  describe "TcgShopBoard" do
    ...
    test "renders shop finances, franchises, market pulse, and customer queue" do
      ...
      assert html =~ "Local Competition"
      assert html =~ "Share"
      assert html =~ "Preorders"
      assert html =~ "Promotions"
      assert html =~ "Organized Play"
      assert html =~ "capacity used"
      assert html =~ "turn-aways"
      assert html =~ "Inventory Aging"
      assert html =~ "Loss Prevention"
      assert html =~ "protection score"
      assert html =~ "Tax Ledger"
      assert html =~ "Cash Handling"
      assert html =~ "cash tenders"
      assert html =~ "bank deposits"
      assert html =~ "drawer counts"
      assert html =~ "over/short"
      assert html =~ "Channel Costs"
      assert html =~ "marketplace fees"
      assert html =~ "Supplier Credit"
      assert html =~ "effective limit"
      assert html =~ "standing"
      assert html =~ "damaged units received"
      assert html =~ "claim credits"
      assert html =~ "Gross Margin"
      assert html =~ "Overhead"
      assert html =~ "Op Profit"
      assert html =~ "Financing"
      assert html =~ "Consignment"
      assert html =~ "Memberships"
      assert html =~ "Sealed Opening"
      assert html =~ "chase hits"
      assert html =~ "Loose Packs"
      assert html =~ "packs on hand"
      assert html =~ "packs prepared"
      assert html =~ "pack revenue"
      assert html =~ "Special Orders"
      assert html =~ "deposit liability"
      assert html =~ "deposits collected"
      assert html =~ "special-order revenue"
      assert html =~ "Refunds"
      assert html =~ "customer returns"
      assert html =~ "restocked units"
      assert html =~ "return writeoff"
      assert html =~ "Store Credit"
      assert html =~ "Failure Modes"
      assert html =~ "No active failure modes"
      assert html =~ "Payroll"
      assert html =~ "Staffing"
      assert html =~ "scheduled labor"
      assert html =~ "Shrink"
    end
  end
